...
#                print area,leftBound,rightBound
    ...
    def getAxeValue(self,ingrname,axe=0):
        ingrpositions=[self.env.molecules[i][0][axe] for i in range(len(self.env.molecules)) if self.env.molecules[i][2].name == ingrname]
    ...
    def getDistance(self,ingrname, center):
        distA=[]
        ingrpositions=[self.env.molecules[i][0] for i in range(len(self.env.molecules)) if self.env.molecules[i][2].name == ingrname]
    ...
    def getDistanceAngle(self,ingr, center):
        ...
        angles=[]
        distA=[]
        ingrpositions=[self.env.molecules[i][0] for i in range(len(self.env.molecules)) if self.env.molecules[i][2].name == ingr.name]
        ingrpositions=numpy.array(ingrpositions)
        ingrrotation=[self.env.molecules[i][1] for i in range(len(self.env.molecules)) if self.env.molecules[i][2].name == ingr.name]
...
#            gridFileIn=wrkDir+os.sep+"fill_grid"
    ...
    def pack(self,seed=20,forceBuild=True,vTestid = 3,vAnalysis = 0,fbox_bb = None):
        ...
        self.env.fill5(seedNum=seed,verbose=0, vTestid = vTestid,vAnalysis = vAnalysis,fbox = fbox_bb)
        t2 = time()
