...
class DBRecipeHandler(object):
    ...
    def update_reference(self, composition_id, referring_comp_id, index, remove_comp_name, update_in_array=False):
        ...
        if doc is None:
            ...
            new_item, new_item_ref = self.db.get_doc_by_id("composition", referring_comp_id)
            update_ref_path = f"firebase:{new_item_ref.path}" 
            if update_in_array:
                self.db.update_elements_in_array(doc_ref, index, update_ref_path, remove_comp_name)
            else:
    ...
    def convert_sub_doc(self, doc_ref):
        ...
        for doc_key, doc_value in doc.items():
            if doc_key == "object" and doc_value.startswith("firebase:"):
                sub_doc_collection, sub_doc_id = self.db.get_collection_id_from_path(doc_value)
                sub_doc, sub_doc_ref = self.db.get_doc_by_id(sub_doc_collection, sub_doc_id)
                ...
                for region_name, region_array in doc["regions"].items():
                    for region_item in region_array:
                        if isinstance(region_item, dict):
                            if "object" in region_item and region_item["object"].startswith("firebase:"):
                                sub_doc_collection, sub_doc_id = self.db.get_collection_id_from_path(region_item["object"])
                                sub_doc, sub_doc_ref = self.db.get_doc_by_id(sub_doc_collection, sub_doc_id)
                                convert_doc["regions"][region_name][region_array.index(region_item)]["object"] = sub_doc["name"]
                        elif isinstance(region_item, str) and region_item.startswith("firebase:"):
                            sub_doc_collection, sub_doc_id = self.db.get_collection_id_from_path(region_item)
                            sub_doc, sub_doc_ref = self.db.get_doc_by_id(sub_doc_collection, sub_doc_id)
                            convert_doc[doc_key][region_name][region_array.index(region_item)] = sub_doc["name"]
        return convert_doc

        
    def divide_recipe_into_collections(self, recipe_meta_data, recipe_data, upload=True):
        ...
        for comp_name in composition:
            ...
            doc, doc_id = self.check_comp_existence(composition, "composition", comp_name)
            if doc_id:
