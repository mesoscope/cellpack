...
class Ingredient(Agent):
    ...
    def move_one_jitter(self, env, index, done_num, current_update):
        ...
        # checks if the point is close to a surface or in a compartment
        inComp = self.checkPointComp(new_position)
        if inComp:
            # check how far from surface ?
            closeS = self.checkPointSurface(new_position, cutoff=self.cutoff_surface)

        if closeS or not inComp:
            current_update += 1
