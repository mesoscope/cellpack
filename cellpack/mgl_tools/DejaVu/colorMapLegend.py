...
class ColorMapLegend(Insert2d):
    ...
    def __init__(self, colormapgui, name="color map legend", check=1, **kw):
        ...
        self.coord2d = deepcopy(
            self.initialPosition
        )  # 2d coordinates in pixels from top left
    ...
    def Set(self, check=1, redo=1, updateOwnGui=True, **kw):
        ...
        if glfFont is not None:
            if glfFont in list(self.glfVectorFontLoadIdDict.keys()):
        ...
        if fontScale is not None:
            assert isinstance(fontScale, int)
        ...
        if numOfLabels is not None:
            assert isinstance(numOfLabels, int)
    ...
    def pickDraw(self):
        ...
        if self.resizeSpot is not None:
            ...
            GL.glVertex2f(
                float(self.resizeSpot[0] + self.resizeSpotRadius),
    ...
    def processHit_cb(self, pick):
        ...
        if (len(pick.hits) == 1) and self in pick.hits:
            if self.viewer.currentObject != self:
                ...
                # the resize button
                self.isMoving = True
                self.clickPosFromLegendBottomLeft = [
                    pick.event.x - self.coord2d[0],
    ...
    def createOwnGui(self):
        ...
        # fontScale
        self.fontScaleThumb = ThumbWheel(
            frame1,
        ...
        # numOfLabel
        self.numOfLabelsCtr = ThumbWheel(
            frame1,
        ...
        self.frameVar.set(1)
        self.checkBoxFrame = tkinter.Checkbutton(
            frame1, text="Frame", variable=self.frameVar, command=self.setWithOwnGui
        )
        self.checkBoxFrame.pack(side="top")
    ...
    def setWithOwnGui(self, event=None):
        ...
        labelValues = list(map(float, string.split(self.labelValsEnt.get())))
