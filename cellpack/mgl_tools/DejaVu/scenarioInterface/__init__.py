...
def getActor(object, propname):
    ...
    if not animatableProps[0].has_key(propname):
        print "DejaVu.scenarioInterface.getActor: cannot create %s actor for object:" % (
            propname,
        ), object
        return None
...
try:
    ...
    from DejaVu.Viewer import (
        SetCurrentObjectEvent,
        SetCurrentCameraEvent,
        SetCurrentLightEvent,
        SetCurrentClipEvent,
        AddObjectEvent,
    )
    ...
    class DejaVuScenario:
        ...
        def setDirector(self):
            if not self.menuCreated:
                ...
                vi.GUI.propertyNoteBook.setnaturalsize(
                    pageNames=(vi.GUI.propertyNoteBook.getcurselection(),)
                )
        ...
        def createCheckbuttons(self, object):
            ...
            if isinstance(object, Geom):
                menuB = self.animObjMenuB
            elif isinstance(object, Camera):
                menuB = self.animCamMenuB
            elif isinstance(object, Light):
                menuB = self.animLightMenuB
            elif isinstance(object, ClippingPlane):
                menuB = self.animClipMenuB
            else:
            ...
            for i, name in enumerate(propnames):
                ...
                menuB.menu.add_checkbutton(label=name, command=cb, variable=var)
            menuB.menu.add_separator()
            cb = CallbackFunction(self.actorFromFile_cb, propnames, object)
            menuB.menu.add_command(label="Create Actor From File", command=cb)
        ...
        def createActor(
            ...
            if check:
                for i, a in enumerate(director.actors):
                    if a.name == actorname:
                        text = (
                            "Actor %s exists. Do you want to overwrite it?\n"
                            % actorname
                        )
                        d = SimpleDialog(
                            director.gui.root,
            ...
            if actorData is not None:
                ...
                actor = FileActor(
                    *(propname, object, actorClass, actorData) + args, **kw
                )
        ...
        def numarr2str(self, val):
            ...
            valstr = (
                "array("
                + numpy.array2string(val, precision=3, separator=",")
                + ", '%s')" % val.dtype.char
            )
            # valstr.replace("\n", "\n\t")
        ...
        def getActorScript(self, actor, indent, actorind):
            ...
            if hasattr(object, "fullName"):
                ...
                lines.append(
                    indent + "obj%d = vi.FindObjectByName('%s')\n" % (actorind, objname)
                )
            ...
            lines.append(
                indent
                + "if not obj%d: tkMessageBox.showwarning('DejaVu Scenario Warning', 'Object %s does not exist.\\nCannot create actor %s')\n"
            ...
            if not isFileActor:
                ...
                lines.append(
                    newindent
                    + "actor%d = sci.createActor(obj%d, '%s', redraw=False)\n"
                    % (actorind, actorind, propname)
                )

                lines.append(
                    indent
                    + "except:  tkMessageBox.showwarning('DejaVu Scenario warning', 'Could not create actor %s')\n"
                    % name
                )
                lines.append(indent + "if actor%d:\n" % actorind)
                lines.append(
                    newindent + "if hasattr(actor%d, 'keyframeValues'):\n" % actorind
                )
                lines.append(
                    newindent + "    actor%d.keyframeValues = vals\n" % actorind
                )
        ...
        def getNewActors(self):
            ...
            for object in vi.rootObject.AllObjects():
                ...
                if object.listed:
                    ...
                    if (
                        object.fullName.find("misc") < 0
                        and object.fullName.find("selection") < 0
                    ):
        ...
        def actorFromFile_cb(self, propnames, object):
            ...
            if object in self.fileactorGui.keys():
                ...
                self.fileactorGui[object] = fileActorGUI(
                    self, propnames, object, director.createActorFromData
                )
