...
class Insert2d(Common2d3dObject):
    ...
    def __init__(self, name="insert2d", check=1, **kw):
        ...
        self.polygonContour = (
            None  # the list of 2d points (pixels coordinates from bottom left)
        )
    ...
    def Set(self, check=1, redo=1, updateOwnGui=True, **kw):
        ...
        size = kw.get("size")
        if size and (size[0] >= 0.0) and (size[1] >= 0.0):
            self.size = size
            redoFlags |= self._redoFlags["redoDisplayListFlag"]
    ...
    def getTile(self):
        ...
        left = Left + (Right - Left) * (
            tr.CurrentColumn * tr.TileWidthNB - tr.TileBorder
        ) / float(tr.ImageWidth)
        right = left + (Right - Left) * tr.TileWidth / float(tr.ImageWidth)
        bottom = Bottom + (Top - Bottom) * (
            tr.CurrentRow * tr.TileHeightNB - tr.TileBorder
        ) / float(tr.ImageHeight)
    ...
    def setPosition(self, event, redo=1):
        ...
        self.position = [
            event.x / float(self.viewer.currentCamera.width),
    ...
    def calculateAnchorAndPosition(self, event):
        insert2dTopLeftFromScreenTopleft0 = (
            self.position[0] * self.viewer.currentCamera.width
            - self.anchor[0] * self.size[0]
        )
        insert2dTopLeftFromScreenTopleft1 = (
            self.position[1] * self.viewer.currentCamera.height
        ...
        self.anchor = [
            (event.x - insert2dTopLeftFromScreenTopleft0) / float(self.size[0]),
    ...
    def processHit_cb(self, pick):
        ...
        if (len(pick.hits) == 1) and self in pick.hits:
