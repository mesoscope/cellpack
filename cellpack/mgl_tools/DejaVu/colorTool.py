...
def glMaterialWithCheck(face, property, material, eps=0.001, check=True):
    ...
    if property == GL.GL_SHININESS:
        ...
        if check:
            newCol = (
                fabs(matMem[0] - material[0]) > eps
                or fabs(matMem[1] - material[1]) > eps
                or fabs(matMem[2] - material[2]) > eps
                or (fabs(matMem[3] - material[3]) > eps and propNum in (0, 1))
            )
        else:
            newCol = True
...
def HSVRamp(size=256, upperValue=0.6666666666666667):
...
def RGBRamp(size=256, upperValue=0.6666666666666667):
...
def RGBARamp(size=256, upperValue=0.6666666666666667):
