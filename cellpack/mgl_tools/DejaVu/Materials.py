...
#
# $Header: /opt/cvs/python/packages/share1.5/DejaVu/Materials.py,v 1.30 2010/11/18 19:35:37 sanner Exp $
#
# $Id: Materials.py,v 1.30 2010/11/18 19:35:37 sanner Exp $
#
...
try:
    ...
    GL = None
import cellpack.mgl_tools.DejaVu.materialsDef as materialsDef
...
class Materials:
    ...
    def __init__(self, propName=None, propInd=None):
        ...
        if preventIntelBug_WhiteTriangles:
            ...
            # This line was causing diffuse to alwas be 60% of diffuse hence
    ...
    def GetProperty(self, prop):
        ...
        if self.getFrom[prop] is None:
            ...
            if type(sourceProp) is bytes:
                ...
                sourceProp = array((1.0, 1.0, 0.0), "f")
                p = (self.prop[sourceProp] * sca).astype("f")
                return self.binding[sourceProp], p
    ...
    def fixOpacity(self):
        ...
        if self.binding[self.opac] == viewerConst.OVERALL or difflen != opaclen:
    ...
    def Set(self, check=1, **kw):
        ...
        if __debug__:
            if check:
                checkKeywords(*("Material", self.keywords), **kw)
    ...
    def getState(self, includeBinding=True):
        ...
        if includeBinding:
            ...
            if (self.binding[0] != viewerConst.PER_VERTEX) and (
                self.binding[0] != viewerConst.PER_PART
            ):
                lDict["ambient"] = self.prop[propertyNum["ambi"]].tolist()
            if (self.binding[1] != viewerConst.PER_VERTEX) and (
                self.binding[1] != viewerConst.PER_PART
            ):
                lDict["diffuse"] = self.prop[propertyNum["diff"]].tolist()
                lDict["opacity"] = self.prop[propertyNum["opac"]].tolist()
            if (self.binding[2] != viewerConst.PER_VERTEX) and (
                self.binding[2] != viewerConst.PER_PART
            ):
                lDict["emission"] = self.prop[propertyNum["emis"]].tolist()
            if (self.binding[3] != viewerConst.PER_VERTEX) and (
                self.binding[3] != viewerConst.PER_PART
            ):
                lDict["specular"] = self.prop[propertyNum["spec"]].tolist()
            if (self.binding[4] != viewerConst.PER_VERTEX) and (
                self.binding[4] != viewerConst.PER_PART
            ):
