...
from cellpack.mgl_tools.mglutil.math.rotax import rotax, rotVectToVect
import numpy
...
def capMesh(vertices, edges, faces, faceEdges, pp, pno, minDist=0.01):
    ...
    for i in range(len(edges)):
        ...
        proj2 = nproj2[i]
        if (
            (proj1 == 0) or (proj2 == 0) or (arad1[i] > halfPi) != (arad2[i] > halfPi)
        ) and (proj1 + proj2 > 0):
    ...
    t0 = time()
    edgeList = list(range(0, len(edgesS)))
    allFaces = []
    allVertices = []
    while len(edgeList):
        t1 = time()
