...
from cellpack.mgl_tools import DejaVu
...
class VertexSet(Set):
    ...
    def __repr__(self):
        ...
        return '<%s> with %d elements prop: %s' % \
              (self.__class__, len(self.vertices), str(list(self.properties.keys())))
...
class FaceSet(Set):
    ...
    def __repr__(self):
        if __debug__:
         if hasattr(DejaVu, 'functionName'): DejaVu.functionName()
        return '<%s> with %d elements prop: %s' % \
                  (self.__class__, len(self.faces), str(list(self.properties.keys())))
