...
class Ellipsoids(Geom):
    ...
    def Set(self, check=1, redo=1, updateOwnGui=True, **kw):
        ...
        if hasattr(self.vertexSet, "scaling") is False:
            self.vertexSet.scaling = datamodel.ScalarProperties(
                "scaling", shape=(0, 3), datatype=viewerConst.FPRECISION
            )

        if hasattr(self.vertexSet, "orientation") is False:
            self.vertexSet.orientation = datamodel.ScalarProperties(
                "orientation", shape=(0, 4, 4), datatype=viewerConst.FPRECISION
            )
    ...
    def Draw(self):
        ...
        for i in range(len(vertices)):
            ...
            GL.glScalef(
                float(scaling[i][0]), float(scaling[i][1]), float(scaling[i][2])
            )
