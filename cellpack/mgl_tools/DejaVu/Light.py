...
# $Header: /opt/cvs/python/packages/share1.5/DejaVu/Light.py,v 1.42 2009/04/24 19:34:38 vareille Exp $
#
...
class LightModel:
    ...
    def Set(self, **kw):
        ...
        if not localViewer is None:
            if localViewer in (True, 1):
                ...
                raise AttributeError("localViewer can only be True or False")
        ...
        if not twoSide is None:
            if twoSide in (True, 1):
                ...
                raise AttributeError("twoSide can only be True or False")
            self.twoSide = twoSide

            self.broadcast()

        if len(kw):
            print("WARNING8: Keyword(s) %s not used" % list(kw.keys()))

    def broadcast(self):
        # print "LightModel.broadcast"
        if self.viewer.rootObject is None:
            return
    ...
    def Reset(self):
        ...
        self.ambient = (0.12, 0.12, 0.12, 1.0)
...
class Light(Transformable):
    ...
    def Set(self, **kw):
        ...
        if not val is None:
            ...
            GL.glLightfv(
                self.num, GL.GL_CONSTANT_ATTENUATION, [self.constantAttenuation]
            )
        ...
        if not val is None:
            ...
            GL.glLightfv(self.num, GL.GL_LINEAR_ATTENUATION, [self.linearAttenuation])
        ...
        if not val is None:
            ...
            GL.glLightfv(
                self.num, GL.GL_QUADRATIC_ATTENUATION, [self.quadraticAttenuation]
            )
        ...
        if not val is None:
            if val is True:
                ...
                raise AttributeError("positional can only be True or False")
            self.positional = val

        val = getkw(kw, "enabled")
        if not val is None:
            if val in (True, 1):
                GL.glEnable(self.num)
            elif val in (False, 0):
                GL.glDisable(self.num)
            else:
                raise AttributeError("enabled can only be True or False")
            self.enabled = val
        ...
        if not val is None:
            if val in (True, False):
                ...
                raise AttributeError("visible can only be True or False")
        ...
        if not val is None:
            if val >= 1:
                ...
                raise AttributeError("lineWidth has to be >= 1")
        ...
        if not val is None:
            if val > 0.0:
                ...
                raise AttributeError("length has to be > 0.0")
        ...
        if len(kw):
            print("WARNING9: Keyword(s) %s not used" % list(kw.keys()))
    ...
    def broadcast(self):
        ...
        for app in self.applyTo:
            ...
            if (
                app and app.winfo_ismapped()
            ):  # needed otherwise tests produce seg fault on mesa
                app.tk.call(app._w, "makecurrent")
                self.apply()
                app.tkRedraw()

        if self.viewer.materialEditor is not None:
            self.viewer.materialEditor.tk.call(
                self.viewer.materialEditor._w, "makecurrent"
            )
            self.apply()
    ...
    def apply(self):
        ...
        if self.spotFlag:
            GL.glLightfv(num, GL.GL_SPOT_DIRECTION, self.spotDirection[:3])
            GL.glLightfv(num, GL.GL_SPOT_EXPONENT, [self.spotExponent])
            GL.glLightfv(num, GL.GL_SPOT_CUTOFF, [self.spotCutoff])
            GL.glLightfv(num, GL.GL_CONSTANT_ATTENUATION, [self.constantAttenuation])
            GL.glLightfv(num, GL.GL_LINEAR_ATTENUATION, [self.linearAttenuation])
            GL.glLightfv(num, GL.GL_QUADRATIC_ATTENUATION, [self.quadraticAttenuation])
    ...
    def FrameTransform(self, camera=None):
        ...
        if self.redirectXform:
            self.redirectXform.FrameTransform(camera)
    ...
    def DrawDirectionalLight(self, camera):
        ...
        GL.glVertex3f(
            float(camera.lookAt[0]), float(camera.lookAt[1]), float(camera.lookAt[2])
        )
        ...
        for light_color in range(100):
            ...
            GLU.gluCylinder(
                pObj,
                0.5 - 2.0 * light_color / 255.0,
                0.5 - 2.0 * (light_color + 1) / 255.0,
                light_color / 255.0,
                20,
                20,
            )
            GL.glTranslatef(0.0, 0.0, float(light_color) / 255.0)
