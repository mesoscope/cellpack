...
class Common2d3dObject:
    ...
    def Set(self, check=1, redo=1, updateOwnGui=True, **kw):
        ...
        if self.viewer:
            self.viewer.currentCamera.tk.call(
                self.viewer.currentCamera._w, "makecurrent"
            )
        ...
        if not val is None:
            ...
            if self.parent:
                ...
                for o in self.parent.children:
                    if o.name == val and o != self:
                        raise RuntimeError(
                            "name %s already used by child of %s geometry"
                            % (val, self.parent.name)
                        )
            ...
            if self.viewer:
                ...
                self.viewer.GUI.occlusionCamera.chooser.add(
                    (self.fullName, "no comment available", self)
                )
        ...
        if val is not None:
            if val in (True, False):
                ...
                raise ValueError("pickable can only be True or False")
    ...
    def redoNow(self, redo, updateOwnGui, redoFlags, setCurrent=True):
        if redo and redoFlags and self.viewer:
            ...
            if (redoFlags & self._redoFlags["redoDisplayListFlag"]) != 0:
                if self not in list(self.viewer.objectsNeedingRedo.keys()):
                    self.viewer.objectsNeedingRedo[self] = None
                redoFlags |= self._redoFlags["redrawFlag"]
            if (redoFlags & self._redoFlags["redoDisplayChildrenListFlag"]) != 0:
                lObjectsNeedingRedo = list(self.viewer.objectsNeedingRedo.keys())
            ...
            if (redoFlags & self._redoFlags["redoViewerDisplayListFlag"]) != 0:
                ...
                redoFlags |= self._redoFlags["redrawFlag"]
            if (
                updateOwnGui is True
                and self.ownGui is not None
    ...
    def AllVisibleObjects(self):
        ...
        if not self.visible:
            return
        obj = [self] + [x for x in self.children if x.visible]
        if len(obj) == 1:
            return obj
        oi = 1
        while oi != len(obj):
    ...
    def hideOwnGui(self):
        ...
        if self.ownGui is not None and self.ownGui.winfo_ismapped() == 1:
