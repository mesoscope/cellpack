...
from cellpack.mgl_tools.mglutil.gui.BasicWidgets.Tk.fileBrowsers import FileOpenBrowser, \
     FileSaveBrowser
from cellpack.mgl_tools.mglutil.gui.BasicWidgets.Tk.RelabelingCascadeMenu import RelabelingCascadeMenu
...
"""
    ...
    def restoreState(self, event=None):
        ...
        if filename:
            exec(compile(open(filename).read(), filename, 'exec'), {'vi':self.viewer, 'mode':'both'})
            self.viewer.Redraw()
    ...
    def saveTransformCurrentGeom(self, event=None, askGUI=1):
        ...
        if askGUI:
            yesno = tkinter.messagebox.askokcancel('Saving Transformation',
    ...
    def loadTransformCurrentGeom(self, event=None, askGUI=1):
        geom = self.viewer.currentObject
        if askGUI:
            yesno = tkinter.messagebox.askokcancel('Restoring Transformation',
    ...
    def applyTransformation_cb(self, event=None, askGUI=1):
        ...
        geom = self.viewer.currentObject
        if askGUI:
            yesno = tkinter.messagebox.askokcancel('Apply Transformation',
