...
#############################################################################
#
# $Header: /opt/cvs/python/packages/share1.5/DejaVu/ViewerGUI.py,v 1.233 2010/10/22 19:38:16 sanner Exp $
#
...
from cellpack.mgl_tools.mglutil.gui.BasicWidgets.Tk.RelabelingCascadeMenu import (
...
from .SelectionGUI import SelectionGUI
import cellpack.mgl_tools.DejaVu as DejaVu


class TwoSlidersGUI(tkinter.Frame):
    ...
    def createWidgets(self):
        ...
        self.nearC = self.draw.create_polygon(
            1, 1, 10, 1, 5, 10, 1, 1, fill="blue", tag="near"
        )
        self.farC = self.draw.create_polygon(
            1, 1, 10, 1, 5, 10, 1, 1, fill="blue", tag="far"
        )
        self.startC = self.draw.create_polygon(
            1, 10, 5, 1, 10, 10, 1, 10, fill="blue", tag="start"
        )
        self.endC = self.draw.create_polygon(
            1, 10, 5, 1, 10, 10, 1, 10, fill="blue", tag="end"
        )
...
class ViewerGUI:
    def __init__(self, viewer, maxLights, maxClip, name="Viewer", nogui=0, master=None):
        ...
        items = [
            "save Viewer's State",
            "Save Object's state",
            "save Viewer and Objects states",
            "restore state",
            "Load Transformation",
            "Save Transformation",
        ...
        cmds = [
            self.saveViewerState_cb,
            self.saveObjectsStates_cb,
            self.saveViewerAndObjectsStates_cb,
            self.restoreState,
            self.loadTransformCurrentGeom,
            self.saveTransformCurrentGeom,
            self.saveReprToFile,
            self.loadReprFromFile,
        ]
        if not nogui:
            items.append("Exit")
            cmds.append(self.Exit_cb)
        for m in range(len(items)):
            self.menuFile.menu.add_command(label=items[m], underline=0, command=cmds[m])
        ...
        for m in range(len(items)):
            self.menuEdit.menu.add_command(label=items[m], underline=0, command=cmds[m])
        # attach pull down menu to button
        ...
        vars = (  ##self.wysiwyg,
            ...
            self.displayValueInObjList,
            self.showHideQuickKeysVar,
        )
        for m in range(len(items)):
            self.menuPref.menu.add_checkbutton(
                label=items[m],
                var=vars[m],
                command=cmds[m],
        ...
        self.menuPref.menu.add_command(
            label="Add Quick Key", command=self.addQuickKey_cb
        )
        ...
        Radiobutton(
            ...
            indicatoron=0,
            selectcolor="#c67171",
            activebackground="#28686b",
        ).pack(side=LEFT)
        Radiobutton(
            lFrameTransformButtons,
            text="Camera",
            variable=self.Xform,
            command=self.TCamera,
            value="Camera",
            anchor=W,
            indicatoron=0,
            selectcolor="#c67171",
        ...
        Label(
            lFrameTransformInfo, text="zoom", relief="sunken", borderwidth=1, anchor="w"
        ...
        for lButtonIndex in (1, 2, 3):
            ...
            self.transformInfo[lButtonIndex].grid(
                row=1, column=lButtonIndex - 1, sticky="w"
            )
        ...
        self.tvolist.setAction(
            event="select", function=self.setCurrentObjectFromTree_cb
        )
        ...
        self.propertyNoteBook = Pmw.NoteBook(
        ...
        def toggleMenu(event):
            if event.widget.menu.winfo_ismapped():
                event.widget.menu.grab_set()
        ...
        for prop in majorProp:
            ...
            self.inheritMenu_b.menu.add_checkbutton(
                label=prop,
                command=f,
                variable=var,
            )
        ...
        for prop in self.cascadeProp:
            ...
            self.inheritMenu_b.menu.add_cascade(
                label=lCascadeMenu.baseLabel, menu=lCascadeMenu
            )
            lFuncCall = CallBackFunction(
                self.setPropMode, lCascadeMenu.baseLabel, lCascadeMenu.cascadeVariable
            )
        ...
        for lab, mode in list(viewerConst.SHADINGS.items()):
            lCascadeMenu.add_radiobutton(
                label=lab,
                variable=lCascadeMenu.cascadeVariable,
                command=lFuncCall,
        ...
        for lab, mode in zip(
            viewerConst.Front_POLYGON_MODES_keys, viewerConst.Front_POLYGON_MODES_values
        ):
        ...
        for lab, mode in zip(
            viewerConst.Back_POLYGON_MODES_keys, viewerConst.Back_POLYGON_MODES_values
        ):
            if not self.viewer.hasOffsetExt:
                continue
            lCascadeMenu.add_radiobutton(
        ...
        def lineWidthFunc():
            self.setPropMode(
                "lineWidth", self.relabelingCascadeMenus["lineWidth"].cascadeVariable
            )
        ...
        def pointWidthFunc():
            self.setPropMode(
                "pointWidth", self.relabelingCascadeMenus["pointWidth"].cascadeVariable
            )
        ...
        for lab, mode in list(viewerConst.srcBFnames.items()):
            lCascadeMenu.add_radiobutton(
                label=lab,
                variable=lCascadeMenu.cascadeVariable,
                command=self.SetBF,
        ...
        lChidrenInheritProp1 = inheritProp + self.cascadeProp
        lChidrenInheritProp2 = moreProp + opacityProp
        # + inheritProp2#+ otherProp +#majorProp
        lChidrenInheritProp = lChidrenInheritProp1 + lChidrenInheritProp2

        self.cInheritMenu_b = Menubutton(
            self.inheritF,
        ...
        self.cInheritMenu_b.menu.add_cascade(
            label="to direct children only", menu=lCascade[0]
        )
        self.cInheritMenu_b.menu.add_cascade(
            label="to children recursively", menu=lCascade[1]
        )
        ...
        lFunc = CallBackFunction(
            self.setInherit_cb, "inheritMaterial", self.inheritVar["inheritMaterial"]
        )
        lCheckbutton = Checkbutton(
            f,
            text="inherit",
        ...
        for label, mode in zip(labels, modes):
            Radio_b1.menu.add_radiobutton(
                ...
                value=mode,
                command=self.relabelingCascadeMenus[
                    "frontPolyMode"
                ].setWithCallbackFunction,
            )
        ...
        b = Button(f, text="Zsort", command=self.zSortPoly)
        b.grid(row=4, column=2)

        # sort polygons along -z direction
        b = Button(f, text="-Zsort", command=self.minuszSortPoly)
        ...
        def setSelectionContourColor_cb(color):
            ...
            self.selectionGUI.contourColorButton.configure(
        ...
        for i in range(maxClip):
            ...
            lTargetDict[lTargetText] = (
                self.viewer.clipP[i].color[:3],
                "RGB",
        ...
        # AUTODEPTHCUE
        b = Button(
            self.CameraProp, text="Auto Depthcue", command=self.AutoDepthcue
        ).pack(fill=X, padx=5)
        ...
        Radiobutton_button = Menubutton(
            self.CameraProp, text="Projection", underline=0, relief="raised"
        )
        Radiobutton_button.bind("<Button-1>", toggleMenu, "+")
        Radiobutton_button.pack(fill=X, padx=5)
        # the primary pulldown
        Radiobutton_button.menu = Menu(Radiobutton_button)
        for v, s in list({0: "Perspective", 1: "Orthographic"}.items()):
            Radiobutton_button.menu.add_radiobutton(
                label=s, var=self.projType, value=v, command=self.SetProjection
            )
        ...
        Radiobutton_button = Menubutton(
            self.CameraProp, text="Scene Antialiasing", underline=0, relief="raised"
        )
        Radiobutton_button.bind("<Button-1>", toggleMenu, "+")
        Radiobutton_button.pack(fill=X, padx=5)
        # the primary pulldown
        ...
        for v in jitter.jitterList:
            Radiobutton_button.menu.add_radiobutton(
                label=str(v), var=self.nbJitter, value=v, command=self.SetJitter
            )

        Radiobutton_button["menu"] = Radiobutton_button.menu

        # NPR OUTLINES
        Checkbutton_button = Menubutton(
            self.CameraProp, text="Cartoon Outlines", underline=0, relief="raised"
        )
        ...
        Checkbutton_button.menu.add_checkbutton(
            label="On", var=self.contourTk, command=self.toggleOutline
        )
        # SET NPR PARAMETERS
        Checkbutton_button.menu.add_command(
            label="Edit Parameters", command=self.showCurveTool
        )
        ...
        lFunc = CallBackFunction(
            self.colorChooser.showColorChooser, "light 1 - diffuse"
        )
        b = Button(
            lmframe,
            text="Light Colors",
        ...
        self.lightOnOffButton = Checkbutton(
            f2,
            text="Light On",
        ...
        b = Checkbutton(
            f2,
            text="Show Lights",
            command=self.LightShow,
            variable=self.showLight,
            width=14,
            onvalue=viewerConst.YES,
            offvalue=viewerConst.NO,
        )
        ...
        for i in range(1, maxClip + 1):
            ...
            for j in range(lenlabm1):
                ...
                lw.append(w)
            Radiobutton(
                f1, variable=self.CurrentClip, value=i, command=self.ClipSelect
            ).grid(row=i + 1, column=lenlab)
        ...
        self.propertyNoteBook.setnaturalsize(pageNames=list(lPropertyPage.keys()))
        ...
        ###################### Bookmarks pane ################################
        ######################################################################
        self.BookmarksProp = Frame(
            lPropertyPage["Bookmarks"], relief=RIDGE, borderwidth=2
        )
    ...
    def _setOMP(self, attr, val=None):
    ...
    def outlineMeshProp_cb(self, event=None):
        obj = self.viewer.currentObject
        ...
        if not self.outlineMeshButtonInitialized:
            ...
            lFuncCall = CallBackFunction(self._setOMP, "factor")
            self.ompfactortw = ThumbWheel(
                frame,
                width=70,
                height=16,
                type=float,
                value=obj.outline.factor,
                callback=lFuncCall,
                continuous=True,
                oneTurn=10.0,
                wheelPad=2,
            ...
            lFuncCall = CallBackFunction(self._setOMP, "unit")
            self.ompunittw = ThumbWheel(
                frame,
                width=70,
                height=16,
                type=float,
                value=obj.outline.unit,
                callback=lFuncCall,
                continuous=True,
                oneTurn=10.0,
                wheelPad=2,
            ...
            lFuncCall = CallBackFunction(self._setOMP, "lineWidth")
            self.omplwtw = ThumbWheel(
                frame,
                width=70,
                height=16,
                type=int,
                value=obj.outline.lineWidth,
                callback=lFuncCall,
                continuous=True,
                oneTurn=10.0,
                wheelPad=2,
            ...
            lFuncCall = CallBackFunction(self._setOMP, "lighting")
            ...
            lFuncCall = CallBackFunction(self._setOMP, "color")
            ...
            self.outlineMeshPanel.deiconify()
            self.updateOMPgui()

    def updateOMPgui(self):
        if self.outlineMeshButtonInitialized:
            ...
            if self.outlineMeshPanel.winfo_ismapped():
                if not hasattr(obj, "outline"):
                    ...
                    self.outlineMeshPanel.title(
                        "outline-mesh properties for " + obj.fullName
                    )
    ...
    def createBookmarkPane(self, master):
        ...
        w = self.reprContainer = Pmw.Group(
            master, tag_pyclass=tkinter.Button, tag_text="Save representation"
        )
    ...
    def renameRepr(self, name):
        ...
        if newname != None and newname != name:
            if newname in self.reprMem:
                from tkinter.messagebox import showwarning

                showwarning(
                    "Warning",
                    "Representation name %s already exists" % newname,
                    parent=self.root,
                )
                return
            ...
            for b in self.reprContainer.interior().grid_slaves():
                if hasattr(b, "name"):
                    if b.name == name:
                        ...
                        b.bind(
                            "<Button-3>",
                            CallBackFunction(self.showReprMenu_cb, newname),
                        )
    ...
    def addReprButton(self, name, icon=True):
        if self.allRepr == 0:
            self.viewsBalloon.bind(
                self.reprContainer._ring, "Right click on image\nto display its menu"
            )
        ...
        if icon == True:
            ...
            b = tkinter.Button(
                master=master, text=name, width=4, height=3, wraplength=50
            )
        self.viewsBalloon.bind(b, name)
        b.name = name
        b.configure(command=CallBackFunction(self.restoreRepr, name, b))
        b.grid(
            row=1 + (len(self.reprMem) - 1) / 5,
            column=(len(self.reprMem) - 1) % 5,
            sticky="w",
        )
        self.propertyNoteBook.setnaturalsize(pageNames=("Bookmarks",))
        self.sframe.pack()
    ...
    def saveReprToFile(self, file=None):
        if len(self.reprMem) == 0:
            ...
            showwarning(
                "Warning",
                "There are no representations in Bookmarks to save",
                parent=self.root,
            )
            return
        if file is None:
        ...
        if file:
            ...
            if len(list(f.keys())):
                f.clear()
            for name in list(self.reprMem.keys()):
    ...
    def loadReprFromFile(self, file=None):
        ...
        if file:
            ...
            if len(self.reprMem):
                ...
                action = dialog.activate(
                    geometry="+%d+%d"
                    % (self.root.winfo_x() + 100, self.root.winfo_y() + 200)
                )
                if action == "Cancel":
                    return
                elif action == "REPLACE":
    ...
    def fillTransformInfo_cb(self, event=None):
        ...
        for lButtonIndex in (1, 2, 3):
            lText = self.viewer.currentCamera.mouseButtonActions[self.Xform.get()][
                lButtonIndex
            ][lModifier]
            if lText == "None":
                lText = ""
    ...
    def addOcclusionCamera(self):
        ...
        self.occlusionCamera = OcclusionCamera(root, self.viewer, width=50, height=50)
    ...
    def dialog(self):
        ...
        #        ok=d.go()
        ok = tkinter.messagebox.askokcancel("Quit?", "Do you Wish to Quit?")
        if ok:
            self.quit_cb()
        else:
            return
...
"""
    ...
    def addObject(self, obj, parent):
        ...
        if not parent:
            ...
            self.tvolist.addNode(parent=None, name=obj.name, object=obj)
            # self.olist.select_set(0,0)

        else:
            self.tvolist.addNode(
                parent=self.tvolist.objToNode[parent], name=obj.name, object=obj
            )
    ...
    def SetCurrentObject(self, obj):
        ...
        for prop, var in list(self.inheritVar.items()):
            if hasattr(obj, prop):
                if (obj is self.viewer.rootObject) and (prop.startswith("inherit")):
                    var.set(False)
                else:
        ...
        if lIsInstanceTransformable:
            ...
            if (obj is not self.viewer.rootObject) and (
                obj.inheritFrontPolyMode is True
            ):
            ...
            elif (obj is not self.viewer.rootObject) and (
                obj.inheritBackPolyMode is True
            ):
                lCascadeMenu.setWithoutCallbackFunction(viewerConst.INHERIT)
            ...
            for prop in self.cascadeProp:
                ...
                if (obj is not self.viewer.rootObject) and (
                    getattr(obj, inheritProp) is True
                ):
                    lCascadeMenu.setWithoutCallbackFunction(viewerConst.INHERIT)
                else:
        ...
        if len(obj.children) == 0:
            self.menuEdit.menu.entryconfig("Apply Transformation", state="normal")
        else:
            self.menuEdit.menu.entryconfig("Apply Transformation", state="disabled")

        if obj == self.viewer.rootObject:
            for lButton in list(self.objectInheritButtons.values()):
            ...
            for lCascadeMenu in list(self.relabelingCascadeMenus.values()):
    ...
    def setChildProp_cb(self, propname, recursive):
        ...
        if propname == "scissor":
            ...
            obj.SetForChildren(*(recursive,), **{propname: propval})
    ...
    def setChildInheritProp_cb(self, propname, recursive):
        ...
        if isinstance(obj, Geom):
            ...
            obj.SetForChildren(*(recursive,), **{inheritPropName: inheritPropVal})
            propVal = getattr(obj, propname)
            obj.SetForChildren(*(recursive,), **{propname: propVal})
            self.viewer.deleteOpenglList()
            self.viewer.Redraw()
    ...
    def CameraVideoRecorder_cb(self):
        ...
        if camera.videoRecorder:
            ...
            camera.videoRecorder = Recorder(
                root, filetypes=[("MPG", ".mpg")], fileName=filename, camera=camera
            )
    ...
    def ClipOnOff(self):
        ...
        if on:
            ...
            if self.clipvar[cpn][1].get():
                side = 1
            else:
                side = -1
            if self.clipvar[cpn][2].get():
                inh = True
            else:
                inh = False
    ...
    def saveTransformCurrentGeom(self, event=None, askGUI=1):
        ...
        if askGUI:
            yesno = tkinter.messagebox.askokcancel(
                "Saving Transformation", 'Saving matrix for geom "%s"?' % geom.name
            )
            if not yesno:
                return
        filename = self.getSaveFileName()
    ...
    def loadTransformCurrentGeom(self, event=None, askGUI=1):
        ...
        if askGUI:
            yesno = tkinter.messagebox.askokcancel(
                "Restoring Transformation", 'Apply matrix on geom "%s"?' % geom.name
            )
            if not yesno:
                return
        filename = self.getLoadFileName()
    ...
    def loadTransformation(self, filename, geom):
        if filename:
            mydict = {}
            self.lastDir = os.path.split(filename)[0]
            exec(compile(open(filename).read(), filename, "exec"), globals(), mydict)
