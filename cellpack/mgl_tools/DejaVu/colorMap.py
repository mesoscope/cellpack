...
class ColorMap:
    ...
    def __init__(
        ...
        **kw
        ...
        if cmap is not None:
            ...
            if name is not None:
                warnings.warn(
                    "color map creation with cmap and name: name won't be used"
                )
            if mini is not None:
                warnings.warn(
                    "color map creation with cmap and mini: mini won't be used"
                )
            if maxi is not None:
                warnings.warn(
                    "color map creation with cmap and maxi: maxi won't be used"
                )
            name = cmap.name
            ramp = cmap.ramp
            labels = cmap.labels
    ...
    def configure(
        ...
        **kw
        ...
        assert type(self.ramp) is list, type(self.ramp)
    ...
    def asHSV(self, redo=0):
        self.hsv = list(map(lambda x, conv=ToHSV: conv(x), self.ramp))
    ...
    def asHSL(self, redo=0):
        ...
        return list(map(lambda x, conv=RGBA2HSLA_list: conv(x), self.ramp))

    def asTKCOL(self, redo=0):
        if redo or not hasattr(self, "tkcol"):
            self.tkcol = list(map(lambda x, conv=TkColor: conv(x), self.ramp))
        return self.tkcol
