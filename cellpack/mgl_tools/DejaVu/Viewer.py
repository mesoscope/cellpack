...
#########################################################################
#
# $Header: /opt/cvs/python/packages/share1.5/DejaVu/Viewer.py,v 1.352 2010/10/04 22:16:40 sanner Exp $
#
# $Id: Viewer.py,v 1.352 2010/10/04 22:16:40 sanner Exp $
#
...
# Needed to specify the numpy that has oldnumberic
import importlib
import sys
...
from cellpack.mgl_tools.mglutil.events import Event, EventHandler
...
from cellpack.mgl_tools.DejaVu.Light import Light, LightModel
from cellpack.mgl_tools.DejaVu.colorTool import OneColor
from cellpack.mgl_tools.DejaVu.tileRenderer import TRctx
from cellpack.mgl_tools.DejaVu.Transformable import Transformable
from cellpack.mgl_tools.DejaVu.Insert2d import Insert2d
from cellpack.mgl_tools.DejaVu.Common2d3dObject import Common2d3dObject
from cellpack.mgl_tools.DejaVu.cursors import cursorsDict
...
class Viewer(tkinter.Widget, tkinter.Misc, EventHandler):
    ...
    def getViewerStateDefinitionCode(
        self, viewerName, indent="", withMode=True, rootxy=True
    ):
        ...
        for i, c in enumerate(self.cameras):
            ...
            lines.append(indent + "state = %s\n" % str(self.cameras[i].fog.getState()))
            lines.append(
                indent + "%s.cameras[%d].fog.Set(**state)\n\n" % (viewerName, i)
            )
        ...
        lines.extend(
            self.rootObject.getGeomMaterialCode("%s.rootObject" % viewerName, indent)
        )

        # rootObject's clipping planes
        lines.extend(
            self.rootObject.getGeomClipPlanesCode("%s.rootObject" % viewerName, indent)
        )

        return lines

    def getObjectsStateDefinitionCode(
        self, viewerName, indent="", withMode=True, includeBinding=True
    ):
        """Returns a list of strings containing Python code to restore the
        ...
        for obj in self.rootObject.AllObjects():
            ...
            if obj._modified:
                ...
                if isinstance(obj, Geom):
                    ...
                    lines.extend(
                        obj.getGeomMaterialCode(
                            "obj", indent, includeBinding=includeBinding
                        )
                    )
    ...
    def AddCamera(
        ...
        **kw
        ...
        if classCamera is None:
            classCamera = Camera
            name = "camera " + str(len(self.cameras))
        else:
            name = classCamera.__name__ + str(len(self.cameras))
        ...
        # kw['takefocus'] = 1
        kw["stereo"] = stereo
        c = classCamera(*(master, screenName, self, len(self.cameras)), **kw)
    ...
    def InitLighting(self):
        ...
        self.lights[0].Set(
            enabled=viewerConst.YES,
            direction=(-1.0, 1.0, 1.0, 1.0),
            diffuse=[1.0, 1.0, 1.0, 1.0],
            specular=(0.8, 0.8, 0.65, 1.0),
        ...
        self.lights[1].Set(
            enabled=viewerConst.YES,
            direction=(9.0, 0.5, 3.0, 1.0),
            diffuse=(0.35, 0.25, 0.175, 1.0),
        ...
        self.lights[2].Set(
            enabled=viewerConst.YES,
            direction=(0.0, -0.45, -0.35, 1.0),
            diffuse=(0.4, 0.5, 0.75, 1.0),
            specular=(0.0, 0.0, 0.0, 1.0),
            ambient=(0.12, 0.12, 0.12, 1.0),
            tagModified=False,
        )
    ...
    def __init__(
        ...
        **kw
        ...
        if not master:
            ...
            assert isinstance(master, tkinter.BaseWidget) or isinstance(
                master, tkinter.Tk
            )
        ...
        self.avgFrameRate = 0.0  # average frame rate between button down and
        ...
        self.redrawLock = _thread.allocate_lock()
        self.objectsNeedingRedo = {}
        ...
        c = Viewer.AddCamera(*(self, master, screenName, classCamera), **kw)
        ...
        if maxLights > 8:
            print("WARNING: Reducing number of light sources from %d to 8" % maxLights)
            maxLights = 8
        ...
        self.GUI = ViewerGUI(self, maxLights, maxClipP, nogui=nogui, master=guiMaster)
    ...
    def saveViewerState(self, filename):
        ...
        f = open(filename, "w")
        dum = list(map(lambda x, f=f: f.write("%s" % x), code))
        f.close()
    ...
    def unsolicitedPick(self, pick):
        ...
        for object, items in list(pick.hits.items()):
            print("Object", object.name, pick.type, items)
    ...
    def getModifier(self):
        if self.kbdModifier["Shift_L"]:
            return "Shift_L"
        elif self.kbdModifier["Control_L"]:
            return "Control_L"
        elif self.kbdModifier["Alt_L"]:
            return "Alt_L"
        elif self.kbdModifier["Shift_R"]:
            return "Shift_R"
        elif self.kbdModifier["Control_R"]:
            return "Control_R"
        elif self.kbdModifier["Alt_R"]:
            return "Alt_R"
        else:
            return "None"
    ...
    def NormalizeCurrentObject(self):
        ...
        if isinstance(lObject, Transformable):
            self.rootObject.Set(scale=(1.0, 1.0, 1.0))
            mini, maxi = lObject.ComputeBB()
            g = Numeric.add.reduce((mini, maxi)) * 0.5  # center of BoundingBox
            ...
            c.lookFrom = Numeric.array(
                (0.0, 0.0, c.nearDefault + lNewDist + lHalfObject)
            )
            c.lookAt = Numeric.array((0.0, 0.0, 0.0))
    ...
    def get3DPointFromPick(self, event):
        ...
        if isinstance(self.currentObject, Transformable):
            ...
            if self.redirectTransformToRoot or self.currentObject == self.rootObject:
                ...
                m = self.currentObject.GetMatrixInverse(root=self.rootObject)
                g = Numeric.dot(m, g)[:3]
            return g, background
    ...
    def _FocusCameraBB(self):
        ...
        dist = (
            self.oldLookFrom * (steps - i - 1.0) / steps + newDist * (i + 1.0) / steps
        )
        cam.lookFrom = Numeric.array((0.0, 0.0, dist))
    ...
    def FocusCameraBBT(self, mini, maxi, focusDuration=0.5, after_cb=None):
        """Moves camera to provided as [mini, maxi] bounding box"""
        g = Numeric.sum((mini, maxi), 0) * 0.5  # center of BoundingBox
        self.rootObject.Set(scale=(1.0, 1.0, 1.0))
        lBox = maxi - mini
        self.lHalfObject = max(lBox) / 2.0
        if self.lHalfObject == 0.0:
            self.lHalfObject = 1.0
        cam = self.currentCamera
        ...
        cam.lookAt = Numeric.array((0.0, 0.0, 0.0))
        Rmini, Rmaxi = self.rootObject.ComputeBB()
        Rg = Numeric.sum((Rmini, Rmaxi), 0) * 0.5
    ...
    def FocusOnBoxT(self, obj, mini, maxi):
        ...
        self.FocusCameraBB(mini, maxi, after_cb=cb)
    ...
    def FocusCameraBB(self, obj, mini, maxi):
        ...
        g = numpy.sum((mini, maxi), 0) * 0.5  # center of BoundingBox
        ...
        # compute
        Rg = numpy.sum((Rmini, Rmaxi), 0) * 0.5
        # print 'Rg', Rg
        ...
        for i in range(steps):
            ...
            lNewDist = lHalfObject / math.tan(cam.fovy * 0.5 * toRad)
            newDist = cam.nearDefault + lNewDist + lHalfObject
            dist = oldLookFrom * (steps - i - 1.0) / steps + newDist * (i + 1.0) / steps
    ...
    def FocusOnBox(self, obj, mini, maxi):
        ...
        try:
            ...
            maa.runIn(1.0, self.master.after)
        except ImportError:
            maa.run()
    ...
    def AddObjectAndParents(self, obj, parent=None, redraw=True, redo=True, local=True):
        ...
        for geom in lListOfAdditions[::-1]:  # going backwards
            if geom.viewer is None:
                if geom.replace is False:
                    geom.name = self.ensureUniqueName(geom.name, geom.parent, local)
                else:
    ...
    def AddObject(self, obj, parent=None, redraw=True, redo=True):
        ...
        assert parent.viewer == self, "ERROR: the parent must be in the same viewer"
        ...
        try:
            self._AddObject(obj, parent, redo=redo)  # replace,
        finally:
            self.redrawLock.release()
            if redraw:
                self.Redraw()

    def _AddObject(self, obj, parent, redo=False):  # replace,
        """Add an object to the viewer
                    ...
                    if obj.replace is not False:  # replace child by obj
                        ...
                        if obj.name[-6:] == "_":
                            try:  # if it had a number already replace it
                                int(obj.name[-5:])
                                obj.name = (
                                    obj.name[:-6] + "_%05d" % self.lastObjectNumber
                                )
    ...
    def _RemoveObjectRec(self, obj):
        ...
        if obj.protected:
            raise RuntimeError(
                "attempt to remove a protected object from the DejaVu viewer: %s" % obj
            )
            return
        ...
        if len(obj.children) == 0 and obj.parent is not None:
            ...
            if obj in list(self.objectsNeedingRedo.keys()):
    ...
    def ReparentObject(self, obj, newparent, objectRetainsCurrentPosition=False):
        ...
        assert (
            newparent.viewer is None or newparent.viewer == self
        ), "Parent is in another viewer"

        assert (
            self.AreYouMyDescendant(obj, newparent) is False
        ...
        if (
            objectRetainsCurrentPosition in (0, False)
            and obj.immediateRendering is False
        ):
            if obj not in list(self.objectsNeedingRedo.keys()):
        ...
        for c in self.cameras:
            ...
            if self.tileRender:
                ...
                for i in range(numOfPasses):
                    ...
                    self.tileRenderBuffer[i] = Numeric.zeros(
                        self.tileRenderFinalWidth * h * 3, Numeric.UnsignedInt8
                    )
                    ...
                    print(
                        "image size",
                        self.tileRenderFinalWidth,
                        self.tileRenderFinalHeight,
                    )
                    tr.imageSize(self.tileRenderFinalWidth, self.tileRenderFinalHeight)

                    tr.imageBuffer(
                        GL.GL_RGB, GL.GL_UNSIGNED_BYTE, self.tileRenderBuffer[i]
                    )
                    # tr.rowOrder(113) # 112
                    ...
                    if c.projectionType == c.PERSPECTIVE:
                        self.tileRenderCtx.perspective(
                            c.fovy, float(c.width) / float(c.height), c.near, c.far
                        )
                    else:
                        self.tileRenderCtx.ortho(
                    ...
                    im[i] = Image.fromstring(
                        "RGB",
                        (self.tileRenderFinalWidth, self.tileRenderFinalHeight),
                ...
                try:
                    if c.stereoMode.startswith("SIDE_BY_SIDE"):
                        ...
                        imageFinal.save(self.tileRenderOutput)
                    else:
                        im[0].save(self.tileRenderOutput)
    ...
    def ReallyRedraw(self):
        ...
        try:
            ...
            if len(self.objectsNeedingRedo):
                ...
                for o in list(self.objectsNeedingRedo.keys()):
                    if hasattr(o, "justGotRedone"):
                        continue
                    o.justGotRedone = True
                    if o.immediateRendering or (
                        self.tileRender and o.needsRedoDpyListOnResize
                    ):
    ...
    def OneRedraw(self):
        ...
        if self.autoRedraw:
            ...
            # raise RuntimeError('OneRedraw shoudl NOT be called while Viewer is in AutoRedraw mode')
    ...
    def BindTrackballToObject(self, obj, allCameras=None):
        ...
        if not isinstance(obj, Common2d3dObject):
            raise AttributeError(
                "first parameter has to be an instance of \
    Comon2d3dObject"
            )
    ...
    def TranslateCurrentObjectXY(self, event, matrix, transXY, transZ):
        ...
        lDistanceFactor = (c.far - c.near) * 0.001
    ...
    def TranslateCurrentObjectZ(self, event, matrix, transXY, transZ):
        ...
        if isinstance(self.currentObject, Transformable):
            ...
            lDistanceFactor = (c.far - c.near) * 0.001
            lDistanceFactor = math.exp(lDistanceFactor) - 1
            lFactor = lDistanceFactor * lFovyFactor * 0.5
            t = (0.0, 0.0, transZ * lFactor)
            if c.currentTransfMode == "Clip":
                self.currentClip.ConcatTranslation((transZ * lFactor, 0.0, 0.0))
            else:
                self.currentObject.ConcatTranslation(t)
            self.Redraw()
    ...
    def CurrentObjectBackColor(self, prop, value):
        ...
        if value:
            self.currentObject.Set(
                materials=(value,), propName=prop, polyFace=GL.GL_BACK
            )
    ...
    def BindTrackballToClip(self, c, allCameras=None):
        ...
        if not isinstance(c, ClippingPlane):
            raise AttributeError(
                "first parameter has to be an instance\
     of ClippingPlane"
            )
    ...
    def ScaleCurrentCamera(self, event, matrix, transXY, transZ):
        ...
        c = self.currentCamera
        value = c.fovy * 0.01 * transZ
        fovy = c.fovy - value
        if (fovy < 180.0) and (fovy > 0.0):
            c.Set(fov=fovy)
        self.Redraw()
    ...
    def CurrentLightColor(self, prop, value):
        ...
        if value:
            if prop == "ambient":
                self.currentLight.Set(ambient=value)
            elif prop == "diffuse":
                self.currentLight.Set(diffuse=value)
            elif prop == "specular":
                self.currentLight.Set(specular=value)
            self.currentCamera.Redraw()
    ...
    def ResetCurrentScissor(self):
        """Reset the current tesxture's transformation"""
        self.currentScissor.Set(scissorX=0, scissorY=0, scissorW=200, scissorH=200)
        self.Redraw()
    ...
    def ScaleCurrentScissor(self, event, matrix, transXY, transZ):
        ...
        sc = self.currentScissor
        sc.Set(
            scissorW=int(sc.scissorW * scaleFactor),
    ...
    def TranslateCurrentScissor(self, event, matrix, transXY, transZ):
        ...
        sc = self.currentScissor
        sc.Set(
            scissorX=int(sc.scissorX + transXY[0]),
    ...
    def deleteOpenglList(self):
        ...
        for c in self.cameras:
            if c is not None and c.dpyList is not None:
                c.tk.call(c._w, "makecurrent")
                currentcontext = self.currentCamera.tk.call(
                    self.currentCamera._w, "contexttag"
                )
    ...
    def removeAllTheDisplayListsExceptTemplatesAndVBO(self):
        ...
        for g in self.rootObject.AllObjects():
            if not (
                hasattr(g, "vertexArrayFlag")
                and g.vertexArrayFlag is True
                and hasattr(g, "vertexSet") is True
    ...
    def transformedCoordinatesWithInstances(self, hits):
        ...
        coordinates"""
        vt = []
        for geom, values in list(hits.items()):
