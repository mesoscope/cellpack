...
if hasattr(DejaVu, "defaultSpinningMode") is False:
    DejaVu.defaultSpinningMode = 0


class Trackball:
    def __init__(
        self, camera, size=0.8, rscale=2.0, tscale=0.05, sscale=0.01, renorm=97
    ):
        ...
        self.spinRotationMat = UnitQuaternion(data=(0.0, -1.0, 0.0, 1.0)).getRotMatrix(
            shape=(16,)
        )
    ...
    def resetSpinValues(self, updateSpinGui=True):
        self.spinAxis = [0.0, 1.0, 0.0]
        self.spinAnglePerFrame = 0.5
        self.rockAngle = 30
    ...
    def ResetBindings(self):
        ...
        for mod in cam.mouseButtonModifiers:
            if mod == "None":
                ...
                mod1 = mod + "-"
            evm.AddCallback(
                "<" + mod1 + "B1-Motion>", eval("self." + mod + "B1motion_cb")
            )
            evm.AddCallback(
                "<" + mod1 + "B2-Motion>", eval("self." + mod + "B2motion_cb")
            )
            evm.AddCallback(
                "<" + mod1 + "B3-Motion>", eval("self." + mod + "B3motion_cb")
            )
        ...
        if lModifier == "None":
            ...
            lModifier += "-"
        evm.AddCallback(
            "<" + lModifier + "ButtonRelease-" + str(lButton) + ">",
            self.launchTrackbalSpin_cb,
        )
        ...
        if vi.GUI.Xform.get() == "Object":
            ...
            if lSpinVar in (1, 2, 3):
                ...
                if lSquaredDistance > 81:
                    ...
                    lUnitQuat = UnitQuaternion(
                        data=(self.tb.quat[3], self.tb.quat[:-1])
                    )
                    self.spinAxis, lAngle = lUnitQuat.getAxisAndAngleDegres()
                    self.spinAnglePerFrame = lAngle / 4
                    self.spinGui.anglePerFrameThumbwheel.set(
                        self.spinAnglePerFrame, update=0
                    )
    ...
    def makeSpinRotationMat(self, spinAxis, spinAngle):
        ...
        # are not the same as those in transformation.py
        lUnitQuat = UnitQuaternion(
            data=(spinAxis[0], spinAxis[1], spinAxis[2], spinAngle)
        )
    ...
    def toggleCycle(self, docb=True):
        ...
        if (self.cancelId == 0) or (self.lastSpinVar != lCurrentSpinVar):
            if lCurrentSpinVar in [0, 1]:
                self.rockCount = self.numOfFrameRock / 2
            self.spinRotationMat = self.makeSpinRotationMat(
                self.spinAxis, self.spinAnglePerFrame
            )
            self.spinCycle()
        else:
            self.camera().restoreAA(None)
    ...
    def oscillateCycle(self):
        ...
        if vi.GUI.Xform.get() == "Object":
            ...
            if self.lastSpinVar == 3:
                ...
                lAtenuationFactor = (
                    math.sin(3.14159 * self.rockCount / lNumOfFrameRock) * 1.57
                )
                self.spinRotationMat = self.makeSpinRotationMat(
                    self.spinAxis, self.spinAnglePerFrame * lAtenuationFactor
                )
                vi.rootObject.ConcatRotation(self.spinRotationMat)
                c.Redraw()
                vi.afterRedraw()
...
""",
        ...
        self.spinGui.vectorGUI = vectorGUI(
            mainFrame,
            size=150,
