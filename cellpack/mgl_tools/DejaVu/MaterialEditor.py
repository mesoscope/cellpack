...
#########################################################################
#
# $Header: /opt/cvs/python/packages/share1.5/DejaVu/MaterialEditor.py,v 1.57 2009/04/10 17:40:46 vareille Exp $
#
# $Id: MaterialEditor.py,v 1.57 2009/04/10 17:40:46 vareille Exp $
#
...
class OGLWidget(tkinter.Widget, tkinter.Misc):
    ...
    def initProjection(self):
        ...
        GL.glLoadIdentity()
        GL.glOrtho(0.0, 1.0, 0.0, 1.0, -2.5, 2.5)
    ...
    def setupLightModel(self):
        ...
        GL.glLightfv(GL.GL_LIGHT1, GL.GL_AMBIENT, [0.5, 0.5, 0.5, 1.0])
...
class MaterialEditor(OGLWidget):
    ...
    def __init__(self, master, viewer, title="MaterialEditor", cnf={}, **kw):
        ...
        self.halfSize = 2
        self.material = [
            Numeric.array([[0.4, 0.4, 0.4, 1.0]], "f"),
            Numeric.array([[0.4, 0.4, 0.4, 1.0]], "f"),
            Numeric.array([[1.0, 1.0, 1.0, 1.0]], "f"),
            Numeric.array([[0.0, 0.0, 0.0, 1.0]], "f"),
            Numeric.array([50], "f"),
            Numeric.array([1.0], "f"),
        ]

        OGLWidget.__init__(*(self, self.frame, title, cnf), **kw)
        ...
        self.opacitySl = ExtendedSlider(
            f,
            label="  Opacity",
            minval=0.0,
            maxval=1.0,
            init=self.material[self.diff][0][3],
            onButton=1,
            sd="left",
            withValue=0,
            immediate=1,
        ...
        self.shiniSl = ExtendedSlider(
            f,
            label="Shininess",
            minval=0.0,
            maxval=128.0,
            init=self.material[self.shini][0],
            onButton=1,
            sd="left",
            withValue=0,
            immediate=1,
        ...
        self.paletteNumBt = Pmw.Counter(
            f1,
            labelpos="nw",
            label_text="Prop number:",
            entry_width=2,
            entryfield_value=self.paletteNum,
            entryfield_validate={"validator": "integer", "min": 0, "max": 34},
        )
        ...
        self.dismissTk = tkinter.Button(self.root, text="DISMISS", command=self.dismiss)
    ...
    def readMaterial(self):
        ...
        file = tkinter.filedialog.askopenfilename(
            filetypes=types, initialdir=self.lastDir, initialfile=None, title=title
        )

        if len(file) == 0:
            return
        d = {}
        exec(compile(open(file).read(), file, "exec"), d)
    ...
    def writeMaterial(self):
        ...
        f = open(file, "w")
        f.write("material = [\n")
        f.write(repr(self.material[self.ambi].tolist()) + ", # ambient\n")
        f.write(repr(self.material[self.diff].tolist()) + ", # diffuse\n")
        f.write(repr(self.material[self.emis].tolist()) + ", # emission\n")
        f.write(repr(self.material[self.spec].tolist()) + ", # specular\n")
        f.write(repr(self.material[self.shini].tolist()) + ", # shininess\n")
        f.write(repr(self.material[self.opac].tolist()) + " # opacity\n")
        f.write("]\n")
        f.close()
    ...
    def broadcast(self):
        ...
        if self.object is None:
            return
        mask = [
            self.ambipe.onTk.get(),
            self.diffpe.onTk.get(),
            self.emispe.onTk.get(),
            self.specpe.onTk.get(),
            self.shiniSl.onTk.get(),
            self.opacitySl.onTk.get(),
        ]

        if self.objectFace == GL.GL_BACK:
            self.object.Set(
                rawMaterialB=self.material, matMask=mask, transparent="implicit", redo=1
            )
        else:
            self.object.Set(
                rawMaterialF=self.material, matMask=mask, transparent="implicit", redo=1
            )
    ...
    def setAmbient(self, rgb):
        ...
        self.material[self.ambi][0][:3] = array(rgb[:3], "f", copy=1)
        self.tkRedraw(self)
        self.broadcast()
    ...
    def setDiffuse(self, rgb):
        ...
        self.material[self.diff][0][:3] = array(rgb[:3], "f", copy=1)
        self.tkRedraw(self)
        self.broadcast()
    ...
    def setEmission(self, rgb):
        ...
        self.material[self.emis][0][:3] = array(rgb[:3], "f", copy=1)
        self.tkRedraw(self)
        self.broadcast()
    ...
    def defineMaterial(self, material, face=None):  # GL.GL_FRONT
        if __debug__:
            if hasattr(DejaVu, "functionName"):
                DejaVu.functionName()
...
class MaterialPropertyEditor(OGLWidget):
    ...
    def __init__(
        ...
        **kw
        ...
        tf = self.topFrame = tkinter.Frame(master, borderwidth=3, relief="ridge")
        ...
        self.setVal = tkinter.Entry(
            self.frame, textvariable=self.valueTk, width=5, bg="#CC9999"
        )
        ...
        cb = tkinter.Checkbutton(f, text=label, variable=self.onTk, command=self.on_cb)
        self.onTk.set(1)
        self.editTk = tkinter.IntVar()
        self.edit = tkinter.Checkbutton(
            f, variable=self.editTk, text="Edit", command=self.edit_cb
        )
        self.setcwTk = tkinter.IntVar()
        self.setcw = tkinter.Button(f, text="SetCW", pady=0, command=self.setcw_cb)
    ...
    def redraw(self):
        ...
        GL.glColor3f(
            float(self.rgbMax[0]), float(self.rgbMax[1]), float(self.rgbMax[2])
        )
        ...
        GL.glVertex2f(float(x1), 0.0)
        GL.glVertex2f(float(x2), 0.0)
        GL.glVertex2f(float(x2), 1.0)
        GL.glEnd()
...
class ColorWheel:
    def __init__(self, master, title=None):
        ...
        self.cwcanvas = tkinter.Canvas(
            f, width=self.width, height=self.height, relief="sunken", borderwidth=3
        )
    ...
    def Set(self, color, mode="HSV"):
        ...
        if mode == "RGB":
            color[:3] = ToHSV(color[:3])
        self.hsvColor = color[:3]

        # update cursor
        rad = self.hsvColor[1] * self.radius
        angle = 2.0 * math.pi * (1.0 - self.hsvColor[0])
...
class ColorChooser:
    def __init__(self, master, title="Color Chooser"):
        ...
        self.chip1 = tkinter.Frame(
            f, relief="sunken", borderwidth=3, width=60, height=30, bg="white"
        )
        self.chip2 = tkinter.Frame(
            f, relief="sunken", borderwidth=3, width=60, height=30, bg="white"
        )
    ...
    def updateSaved(self, color):
        if __debug__:
            if hasattr(DejaVu, "functionName"):
                DejaVu.functionName()
        self.chip2.configure(bg=TkColor(Numeric.array(color, "f") * 255))
