...
#
# $Header: /opt/cvs/python/packages/share1.5/DejaVu/Displayable.py,v 1.56 2010/09/28 17:20:07 sanner Exp $
#
# $Id: Displayable.py,v 1.56 2010/09/28 17:20:07 sanner Exp $
#
...
class OutLine:
    ...
    def Set(self, redo=0, **kw):
        ...
        if len(kw):
            print("WARNING2: Keyword(s) %s not used" % list(kw.keys()))
    ...
    def __init__(self, geom, **kw):
        ...
        if len(kw):
            print("WARNING3: Keyword(s) %s not used" % list(kw.keys()))
...
class LineStipple:
    ...
    def Set(self, **kw):
        ...
        if len(kw):
            print("WARNING4: Keyword(s) %s not used" % list(kw.keys()))
    ...
    def __init__(self, **kw):
        ...
        self.Reset()
        self.Set(factor=getkw(kw, "factor"), pattern=getkw(kw, "pattern"))

        if len(kw):
            print("WARNING5: Keyword(s) %s not used" % list(kw.keys()))

    def __repr__(self):
        if __debug__:
            if hasattr(DejaVu, "functionName"):
                DejaVu.functionName()
        return "<%s> factor=%f pattern=%s" % (
            self.__class__,
            self.factor,
            hex(self.pattern),
        )
...
class PolygonStipple:
    ...
    def Set(self, **kw):
        ...
        if len(kw):
            print("WARNING6: Keyword(s) %s not used" % list(kw.keys()))
    ...
    def __init__(self, **kw):
        ...
        if len(kw):
            print("WARNING7: Keyword(s) %s not used" % list(kw.keys()))

    def __repr__(self):
        if __debug__:
            if hasattr(DejaVu, "functionName"):
                DejaVu.functionName()
        return "<%s>\n pattern=%s" % (self.__class__, list(map(hex, self.pattern)))
...
class Displayable:
    ...
    def __init__(self):
        ...
        self.materials = {
            GL.GL_FRONT: Materials.Materials(),
        ...
        self.flashInDuration = 0.5  # length of flash in seconds
        self.flashSolidDuration = 0.2  # length of flash in seconds
        self.flashOutDuration = 0.8  # length of flash in seconds
    ...
    def _fadeOutT(self):
        ...
        val = exp(1.0) / exp(1.0 + 2 * fraction)
        print(fraction, val)
    ...
    def fadeInT(self, duration=0.5, interval=10):
    ...
    def flashT(
        self, durationIn=0.5, duration=0.2, durationOut=0.8, interval=10, after_cb=None
    ):
    ...
    def fadeOut(self, nbf=25):
        vi = self.viewer
        for i in numpy.arange(1.0, 3.0, 2.0 / nbf):
            val = exp(1.0) / exp(i)
            self.Set(opacity=val)
            self._setTransparent("implicit")
            vi.OneRedraw()
    ...
    def fadeIn(self, nbf=15):
        ...
        vi = self.viewer
        for i in numpy.arange(0, pi * 0.5, (pi * 0.5) / nbf):
            val = sin(i)
            self.Set(opacity=val)
            self._setTransparent("implicit")
            vi.OneRedraw()
        ...
        if self.GetLighting():
            ...
            if shading != GL.GL_NONE:
                if self.normals is not None:
                    if (
                        shading == GL.GL_SMOOTH
                        and len(self.normals) != len(self.vertexSet)
                    ) or (
                        shading == GL.GL_FLAT and len(self.normals) != len(self.faceSet)
                    ):
                        self.GetNormals()
