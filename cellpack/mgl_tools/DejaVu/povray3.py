...
#
# $Header: /opt/cvs/python/packages/share1.5/DejaVu/povray3.py,v 1.10 2009/03/13 18:45:06 vareille Exp $
#
# $Id: povray3.py,v 1.10 2009/03/13 18:45:06 vareille Exp $
#
...
"""
    ...
    def addCamera(self, camera, scaleLight=2.0):
        ...
        str = (
            ...
            + "\n  location"
            + self.coord3(
                camera.lookFrom[0], camera.lookFrom[1], camera.lookFrom[2] + 10
            )
        )
    ...
    def addTexture(self, texture, col):
        ...
        """
        str = "  texture {\n"
        if not "pigment" in list(texture.keys()):
            str = str + "      pigment { color rgb %s }\n" % self.vec3(col)
        elif texture["pigment"] == "":
            str = str + "      pigment { color rgb %s }\n" % self.vec3(col)
        else:
            str = str + "      pigment { %s }\n" % texture["pigment"]
        for k, v in list(texture.items()):
            if k == "pigment":
                continue
            if v is None:
                str = str + "      %s\n" % (k,)
    ...
    def addIndexedPolylines(self, geom, texture, bondRad):
        ...
        for j in range(len(lines)):
            ...
            self.entries.append(
                "cylinder{%s,%s, %f open\n"
                % (self.coord3(v[l[0]]), self.coord3(p2), bondRad)
            )
            ...
            if not oneCyl:
                self.entries.append(
                    "cylinder{%s, %s, %f open\n"
                    % (self.coord3(p2), self.coord3(v[l[1]]), bondRad)
                )
    ...
    def addSpheres(self, geom, texture):
        ...
        self.entries.append(
            "#declare unitSphere = sphere { <0.00, 0.00, 0.00> , 1 }\n\n"
        )
    ...
    def addCylinders(self, geom, texture):
        ...
        for i in range(len(geom.faceSet)):
            for j in range(len(f[i]) - 1):
                ...
                if not geom.oneRadius:
                    r1 = radii[f[i][j]] / 2.0
                    r2 = radii[f[i][j + 1]] / 2.0
                else:
                    r1 = r2 = radii[0] / 2.0

                self.entries.append(
                    "\t%s, %f, %s, %f\n"
                    % (self.coord3(v[f[i][j]]), r1, self.coord3(v[f[i][j + 1]]), r2)
                )
...
if __name__ == "__main__":
    ...
    s = Spheres("test", centers=((0.0, 0.0, 0.0), (3.0, 0.0, 0.0)))
    s.Set(quality=4)
    vi.AddObject(s)
