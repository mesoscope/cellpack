...
class blenderUI:
    ...
    def internal_createMenu(self, MenuOp, menuDic, k, sub="", menuOrder=None):
        ...
        for i, elem in enumerate(lelem):
            if sub == elem["name"]:
                ...
                for sub in elem["sub"]:
                    layout.operator(
                        "pyubic.push", text=elem["sub"][sub]["name"]
                ...
                if elem["sub"] is not None:
                    ...
                    layout.operator("pyubic.push", text=elem["name"]).modetype = (
        ...
        #            setattr(self,propName+"_o",None)
            ...
            if elem["type"] == "pullMenu":
                ...
                if elem["action"] is not None:
                    ...
                    setattr(
                        ...
                        propName,
                        bpy.props.EnumProperty(
                            items=lMenu, name=elem["label"], update=updateF
                        ),
                    )
            ...
            #                setattr(self,propName+"_o",None))
                ...
                setattr(
                    bpy.types.Scene,
                    propName,
                    bpy.props.BoolProperty(name=elem["label"], update=updateF),
                )
            ...
            #                setattr(bpy.types.Scene,propName+"_o",bpy.props.IntProperty())
                ...
                setattr(
                    ...
                    propName,
                    bpy.props.IntProperty(
                        name=elem["label"], default=value, update=updateF
                    ),
                )
                setattr(self, propName + "_o", value)
        #                setattr(bpy.types.Scene,propName+"_o",bpy.props.IntProperty())
        ...
        #            setattr(bpy.types.Scene,propName+"_o",bpy.props.StringProperty())
            ...
            if elem["type"] == "frame":
                ...
                setattr(
                    ...
                    propName + "_o",
                    bpy.props.BoolProperty(
                        name=elem["label"], default=value, update=checkUIVar
                    ),
                )
                setattr(self, propName + "_o", bool(value))
    ...
    def drawButton(self, elem, x, y, w=None, h=None):
        ...
        self.current_layout.operator("pyubic.push", text=elem["name"]).modetype = (
            self.uiname + "." + str(elem["id"])
        )
    ...
    def drawLine(self, elem, x, y, w=None, h=None):
        ...
        BGL.glColor3f(
            TitleColor[0] / 256.0, TitleColor[1] / 256.0, TitleColor[2] / 256.0
        )
        endGap = 5
        w, h = self.checkwh(elem, w, h)
        if elem["value"] == "H":
    ...
    def drawQuestion(self, title="", question=""):
        ...
        for line in question.split("\n"):
            if len(line) > 30:
                ...
                block.append(line)
        bpy.ops.pyubic.dialog_message(
            "INVOKE_DEFAULT", messageString=question, width=300, height=300
        )
