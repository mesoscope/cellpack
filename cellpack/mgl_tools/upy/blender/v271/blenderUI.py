...
class FILEBROWSER_PT_General(bpy.types.Operator):
    ...
    filepath = bpy.props.StringProperty(
        name="File Path", description="File path to write file to"
    )
    filter_folder = bpy.props.BoolProperty(
        name="Filter folders", description="", default=True, options={"HIDDEN"}
    )
    filter_python = bpy.props.BoolProperty(
        name="Filter python", description="", default=True, options={"HIDDEN"}
    )
...
class MENU_OT_General(bpy.types.Menu):
    ...
    def draw(self, context):
        ...
        print(
            "guiname", guiname, self.bl_idname
        )  # upy.menu_Test_Spheres2D_Filler_v1_0ui_File
        ...
        if gui.MENU_ID:
            ...
            gui.internal_createMenu(
                self, gui.MENU_ID, k, sub=sub, menuOrder=gui.menuorder
            )
...
class blenderUI:
    ...
    def no_action(self, *args, **kwargs):
        pass
    ...
    def internal_createMenu(self, MenuOp, menuDic, k, sub="", menuOrder=None):
        ...
        for i, elem in enumerate(lelem):
            if sub == elem["name"]:
                ...
                for sub in elem["sub"]:
                    layout.operator(
                        "upy.push", text=elem["sub"][sub]["name"]
                    ).modetype = (self.uiname + "." + str(elem["sub"][sub]["id"]))
                break
            else:
                if elem["sub"] is not None:
                    setattr(
                        bpy,
                        self.title.replace(" ", "_") + "menu_current",
                        k + "." + elem["name"],
                    )
                    idname = "upy.sub_menu_%s_%d" % (self.uiname, i)
                    ...
                    layout.operator("upy.push", text=elem["name"]).modetype = (
                        self.uiname + "." + str(elem["id"])
                    )
        ...
        #                setattr(bpy.types.Scene,propName+"_o",bpy.props.FloatProperty())
            ...
            setattr(
                ...
                bpy.props.StringProperty(
                    default=value, update=updateF, description=elem["tooltip"]
                ),
            )
            setattr(self, propName + "_o", value)
        #            setattr(bpy.types.Scene,propName+"_o",bpy.props.StringProperty())
            ...
            if elem["type"] == "frame":
                setattr(
                    ...
                    bpy.props.BoolProperty(
                        name=name,
                        description="Collapse the Frame",
                        default=bool(elem["collapse"]),
                    ),
                )
            else:
                ...
                setattr(
                    ...
                    propName,
                    bpy.props.BoolProperty(
                        default=value, update=checkUIVar, description=elem["tooltip"]
                    ),
                )
                setattr(self, propName + "_o", bool(value))
    ...
    def drawButton(self, elem, x, y, w=None, h=None):
        ...
        if elem["show"]:
            self.current_layout.operator("upy.push", text=name).modetype = (
                self.uiname + "." + str(elem["id"])
            )
    ...
    def drawButtonToggle(self, elem, x, y, w=None, h=None):
        ...
        if self.subdialog:
            elem["variable"] = self.check_addAction(
                Blender.Draw.Toggle, elem["action"], *args
            )
        else:
    ...
    def resetPMenu(self, elem):
        ...
        if elem["action"] is not None:
            ...
            setattr(
                ...
                propName,
                bpy.props.EnumProperty(
                    items=lMenu, name=elem["name"], update=checkUIVar
                ),
            )
    ...
    def drawError(self, errormsg=""):
        ...
        bpy.ops.upy.dialog_message(
            "INVOKE_DEFAULT", messageString="ERROR: " + errormsg, width=300, height=300
        )
        return
    ...
    def drawQuestionOld(self, title="", question=""):
        ...
        #            else :
        #                block.append(line)
        res = bpy.ops.upy.dialog_message(
            "INVOKE_DEFAULT",
    ...
    def drawMessage(self, title="", message=""):
        ...
        bpy.ops.upy.dialog_message(
            "INVOKE_DEFAULT", messageString=message, width=300, height=300
        )
    ...
    def drawInputQuestion(self, title="", question="", callback=None):
        ...
        clascode += "bpy.utils.register_class(Dialog%s)\n" % classname
        clascode += (
            "bpy.ops.%s('INVOKE_DEFAULT',messageString='%s',answerString='')\n"
            % (idname, question)
        )  # INVOKE_AREA
        exec(clascode, gdic, ldic)
        return True
    ...
    def drawSliders(self, elem, x, y, w=None, h=None):
        ...
        w, h = self.checkwh(elem, w, h)
        scn = bpy.context.scene
        self.current_layout.prop(
            scn, elem["name"].replace(" ", "_"), slider=True, event=True
        )
    ...
    def drawImage(self, elem, x, y, w=None, h=None):
        ...
        Blender.Draw.Image(
            img, x, y - h - 10
        )  # , zoomx=1.0, zoomy=1.0, clipx=0, clipy=0, clipw=-1, cliph=-1)
    ...
    def drawTab(self, bloc, x, y):
        ...
        bloc["variable"] = Blender.Draw.Toggle(
            bloc["name"],
            bloc["id"],
            x,
            y,
        ...
        if scroll:
            if "scrollbar" not in bloc:
                ...
                bloc["scrollbar"]["value"] = ReplacementScrollbar(
                    0,
                    0,
                    100,
                    bloc["scrollbar"]["up"],
    ...
    def drawFrame(self, bloc, x, y):
        ...
        if collapse:
            toprow.prop(scn, cname, icon="TRIA_RIGHT", icon_only=False, emboss=False)
        else:
            toprow.prop(scn, cname, icon="TRIA_DOWN", icon_only=False, emboss=False)
    ...
    def UpdateMenuData(self, blocklayout):
        ...
        if blocklayout is not None:
            ...
            for index, item in enumerate(blocklayout["elems"]):
                ...
                if buttonCount and index > buttonCount:
                    break
    ...
    def SubFrame(self, blocklayout, x=0, y=100, width=200):
        ...
        TextColor = [
            ...
            0,
            0,
        ]  # Theme.Get()[0].get("BUTS").text_hi    #Color of submenu text
        TextColor2 = [
            0,
            0,
            0,
        ]  # Theme.Get()[0].get("ui").menu_text    #Color of menu text
        TabColor = [
            TitleColor[0] / 4 * 3,
            TitleColor[1] / 4 * 3,
            TitleColor[2] / 4 * 3,
        ]  # Color of System submenu tabs
        BackColor = (
            Theme.Get()[0].get("BUTS").panel
        )  # Color of System submenu backgrounds
        #        print blocklayout["wcolor"]
        blocklayout["wcolor"] = [1, 1, 1, 1]
        # Collapse button if left mouse click in title subWindow title area
        if (
            self.MousePos[0] > x
            and self.MousePos[0] < x + width
            and self.MousePos[1] > y - TitleHeight
            and self.MousePos[1] < y
        ):
        ...
        # Draw Tab
        BGL.glColor3f(
            TabColor[0] / 256.0 * blocklayout["wcolor"][0],
        ...
        if not blocklayout["collapse"]:
            ...
            BGL.glColor4f(
                BackColor[0] / 256.0 * blocklayout["wcolor"][0],
                ...
                for index, item in enumerate(block):
                    ...
                    if item["show"]:
                        ...
                        if item["type"] == "line":  # If this is a line
                            if Trigger:
                                ButtonHeight = LineGap
                            else:
                                ButtonHeight = WindowGap
                        else:
                            ButtonHeight = (
                                item["height"] * self.scale
                            )  # 18   #Otherwise this is a button
                        ...
                        if item["type"] == "button":
                            ...
                            self.drawButton(
                                item, ButX, PosInc, w=ButWidth, h=ButtonHeight
                            )
