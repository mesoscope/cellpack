...
class template(uiadaptor):
    ...
    def initWidget(self, id=None):
        ...
        self.submenu = {}
        for i in range(10):
            self.submenu[str(self.id - 1)] = self._addElemt(
                name="file" + str(i), action=self.recentfile_cb
            )
        ...
        self.MENU_ID = {
            ...
            "Edit": [self._addElemt(name="Options", action=self.options_cb)],
            "Help": [
                self._addElemt(name="About", action=self.drawAbout),  # self.drawAbout},
                self._addElemt(
                    name="Help", action=self.drawHelp
                ),  # self.launchBrowser},
            ],
        }
