...
class GamerUI(uiadaptor):
    def setup(self, *args, **kwargs):
        ...
        self.setupLayout(
            kwargs.get("use_surface_mesh_generation", True),
    ...
    def setupLayout(
        self,
        use_surface_mesh_generation,
        use_meshimprovement,
        use_boundary_marking,
    ...
    def load_from_registry(self):
        ...
        def from_registry(registry_dict, guiparams):
            for key, value in guiparams.items():
                ...
                if key == "domains":
                    self.domains = values
                elif "id" in value and "name" in value and key in registry_dict:
    ...
    def initWidget(self, id=None):
        ...
        self.buttons["import_surface_mesh"] = self._add_button(
            "Import Surface Mesh",
            action=self.import_surface_mesh,
            tooltip="Import Surface mesh",
        )

        self.mesh_params["surface_mesh_menu"] = self._addElemt(
            name="Surface mesh:",
            action=self._update_mesh_import,
            value=self.mesh_menu_items,
            width=self._width,
            height=self._height,
        ...
        self.buttons["coarse_dense"] = self._add_button(
            "Coarse Dense",
            action=self.action_closure("coarse_dense"),
        ...
        self.buttons["coarse_flat"] = self._add_button(
            "Coarse Flat",
            action=self.action_closure("coarse_flat"),
        ...
        self.buttons["smooth"] = self._add_button(
            "Smooth", action=self.action_closure("smooth"), tooltip="Smooth the mesh"
        )
        ...
        self.gparams["smooth"]["preserve_ridges"] = self._add_check_box(
            "Preserve ridges", action=None, tooltip="preserve ridges during smoothing"
        )

        # Declare UI elements for normal smooth
        self.buttons["normal_smooth"] = self._add_button(
            "Normal Smooth",
            action=self.action_closure("normal_smooth"),
        ...
        self.buttons["deselect_boundary"] = self._add_button(
            "Deselect",
            action=self._deselect_boundary,
        ...
        self.mparams["name"] = self._addElemt(
            name="",
            action=self._update_boundary_name,
        ...
        self.mparams["color"] = self._addElemt(
            name="Color",
            action=self._update_boundary_color,
        ...
        self.mparams["boundary_menu"] = self._addElemt(
            name="boundary_menu",
            action=self._update_boundary_menu,
            value=self.menu_items,
            width=self._height,
            height=self._height,
            variable=self.addVariable("int", 1),
            type="pullMenu",
        ...
        self.tetparams["domain_menu"] = self._addElemt(
            name="Domains:",
            action=self._update_domain_menu,
            value=self.domain_menu_items,
            width=self._width,
            height=self._height,
            variable=self.addVariable("int", 1),
            type="pullMenu",
        ...
        self.tetparams["domainmarker"] = self._add_int(
            ...
            maxi=10000,
            init=1,
            action=self._update_domain,
            tooltip="A marker value for a domain",
        )

        self.tetparams["use_volume_constraint"] = self._add_check_box(
            "Use volume constraint",
            action=self._update_domain,
            tooltip="Use volume constraint, when generating tetrahedral " "mesh",
            width=self._width,
        )
        ...
        self.tetparams["volume_constraint"] = self._add_float(
            ...
            action=self._update_domain,
            init=100.0,
            tooltip="The volume of a domain",
            step=1,
            precision=1.0,
            width=self._width,
        )
    ...
    def _update_boundary_menu(self):
        ...
        self.setVal(
            self.mparams["color"], (boundary["r"], boundary["g"], boundary["b"])
        )

    def _update_boundary_marker(self):
        if len(self.menu_items) == 0:
            return
        old_name = self.menu_items[self.getLong(self.mparams["boundary_menu"])]
    ...
    def _update_boundary_name(self):
        ...
        for key in boundaries[old_name]["faces"]:
            boundaries[new_name]["faces"][key] = boundaries[old_name]["faces"][key]
    ...
    def _update_domain_menu(self):
        ...
        name = self.domain_menu_items[self.getLong(self.tetparams["domain_menu"])]
        ...
        self.setVal(
            self.tetparams["use_volume_constraint"], domain["use_volume_constraint"]
        )
    ...
    def _update_domain(self):
        ...
        domain["volume_constraint"] = self.getVal(self.tetparams["volume_constraint"])

        # Update GUI
        self._update_domain_frame(domain)

    def _update_domain_frame(self, domain):
        self._layout[self._tetrahedarlization_index] = self.tetrahedralization_frame(
            domain
        )
        self.updateViewer()
    ...
    def repair_mesh(self):
        ...
        for edge in non_manifold_edges:
            for face in edge_map[edge]:
                ...
                open_face_edges = [
                    face_edge
                    for face_edge in self.helper.getFaceEdges(obj, face)
        ...
        for vert in open_vertices:
            ...
            if (
                vert in open_vertices_map
                and open_vertices_map[vert][0] in open_vertices_map
                and open_vertices_map[vert][1] in open_vertices_map
                and open_vertices_map[open_vertices_map[vert][0]][0] == vert
                and open_vertices_map[open_vertices_map[vert][1]][0] == vert
            ):
    ...
    def triangulate_holes(self):
        ...
        for edge_loop in edge_loops:
            ...
            axis_most_variations = np.fromiter(
                set(range(3)).difference([axis_least_variation]), dtype=int
            )
            ...
            for face in surf.faces():
                ...
                for index in [face.a, face.b, face.c]:
                    ...
                    if index < num_vertex_in_edge_loop:
                        ...
                        new_face.append(vertex_offset + index - num_vertex_in_edge_loop)
        ...
        try:
            ...
            from pickle import dump
        dump(
            dict(edge_loops=edge_loops, coordinates=all_coordinates),
    ...
    def import_surface_mesh_from_file_action(self, filename):
        ...
        if file_type == "PDB (Surface)":
            ...
            gmesh.centralize()
            self.gamer_to_host(
                gmesh, {}, filename.replace(".pdb", ""), switch_layer=False
            )
            ...
            self.gamer_to_host(
                gmesh, {}, filename.replace(".off", ""), switch_layer=False
            )
    ...
    def tetrahedralize(self):
        ...
        self.saveDialog(
            "Tetrahedralize",
            self.tetrahedralize_action,
            suffix="*" + self.tetmesh_suffices[format_],
        )  # should be a save dialog
    ...
    def tetrahedralize_action(self, filename):
        ...
        for i, (name, domain) in enumerate(self.domains.items()):
            ...
            if obj is None:
                self.drawError(
                    errormsg="The domain: '%s' is not a mesh in " "this scene" % name
                )
        ...
        # Tetrahedralize mesh
        quality_str = "q%.1fqq%.1faA" % (
            self.getVal(self.tetparams["aspect_ratio"]),
    ...
    def gamer_to_host(
        self, gmesh, boundaries, mesh_name="gamer_improved", switch_layer=True
    ):
        ...
        un_selected_vertices = [
            i for i, gvert in enumerate(gmesh.vertices()) if not gvert.sel
        ]
        faces = [(gface.a, gface.b, gface.c) for gface in gmesh.faces()]
        markers = [
            (i, gface.m) for i, gface in enumerate(gmesh.faces()) if gface.m != -1
        ]
        ...
        if self.getVal(self.gparams["create_new_mesh"]):
            ...
            obj, bmesh = self.helper.createsNmesh(
                mesh_name, verts, None, faces, smooth=0
            )
    ...
    def action_closure(self, action):
        ...
        def action_func():
            ...
            if gmesh is None:
                return
            kwargs = dict(
                (parname, self.getVal(value))
                for parname, value in self.gparams[action].items()
            )
    ...
    def _add_int(
        self, name="int", action=None, init=0, mini=0, maxi=1, tooltip="", **kwargs
    ):
        call_kwargs = self.elemt_kwargs.copy()
        call_kwargs.update(kwargs)
        return self._addElemt(
