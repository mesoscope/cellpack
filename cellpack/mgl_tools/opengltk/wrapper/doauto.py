...
def gl_auto():
    ...
    glGets += [
        ("glGetTexLevelParameter", x, [("target", "GLenum"), ("level", "GLint")])
        for x in "fi"
    ]
    glGets += [("glGetTexParameter", x, [("target", "GLenum")]) for x in "fi"]

    def glGetNames(glgetlist):
        from operator import add

        return reduce(add, [["%s%s" % x[:2], "%s%sv" % x[:2]] for x in glgetlist])

    def argsdocpr(args):
        return string.join(["%s," % x[0] for x in args]), string.join(
            ["%s - %s" % x for x in args], "\n    "
        )
    ...
    def glGetFunv(name, typdesc, args):
        ...
        return string.join(
            [
                ...
                "    result = Numeric.zeros( glGetXXDim[ pname], %s)" % typ,
                "    _gllib.%s, result)" % vcall,
                "    return result",
                "\ndef %s(%s pname):" % (fname, argprint),
                '    """\n    %s' % argsdoc,
                'return Numeric array( %s) or %s if singleton\n"""' % (typ, typ),
                "    result = %s)" % vcall,
                "    if 1 == len( result):",
                "        return result[ 0]",
                "    else:",
    ...
    sztypv += [("glRasterPos", dim, typ) for dim in [2, 3, 4] for typ in "dfis"]
    sztypv += [("glTexCoord", dim, typ) for dim in [1, 2, 3, 4] for typ in "dfis"]
    sztypv += [("glVertex", dim, typ) for dim in [2, 3, 4] for typ in "dfis"]
    ...
    for fun, typ in outdimarrays:
        print "\ndef %s( n):" % fun
        print '    """\nreturn - sequence( %s, n)\n    """' % typ
        print "    result = Numeric.zeros( n, %s)" % typ
    ...
    def rectvfun(name, dim, styp):
        ...
        strs = [
            ...
            '    """\n    v1, v2 - seq( %s, %i)\n"""' % (ctype, dim),
            "    if %i != len( v1):" % dim,
            '        raise TypeError( len( v1), "%i-array expected for v1")' % dim,
            "    if %i != len( v2):" % dim,
            '        raise TypeError( len( v2), "%i-array expected for v2")' % dim,
...
def gl_deprec():
    ...
    deprfuns += [
        (
            "glGetPointerv",
            "( pname, params)",
            "convert cobject to appropriate Numeric array (need size...)",
        )
    ]
