...
class ThumbWheelBaseTest(unittest.TestCase, Dummyevent):
    ...
    def test_setReportDelta(self):
        ...
        pause()
        for i in range(10):
            tw.computeNewValue(1.0)  # emulate mouse rotation
        self.assertEqual(tw.value, 10.0)
    ...
    def test_thumbwheel_invalid_width(self):
        """tests thumbwheel invalid size"""
        root = tkinter.Tk()
        self.assertRaises(AssertionError, ThumbWheel, width="hai", master=root)
        root.destroy()

    def test_thumbwheel_invalid_callbacks(self):
        """tests thumbwheel invalid callbacks"""
        root = tkinter.Tk()
        self.assertRaises(AssertionError, ThumbWheel, callback="jlsd", master=root)

        self.assertRaises(
            AssertionError, ThumbWheel, callback=["jlsd", "afd"], master=root
        )
        root.destroy()
    ...
    def test_thumbwheel_invalid_precision(self):
        ...
        root = tkinter.Tk()
        self.assertRaises(AssertionError, ThumbWheel, precision="jhgdfj", master=root)
        root.destroy()
