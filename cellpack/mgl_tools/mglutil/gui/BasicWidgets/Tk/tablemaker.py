...
class TableManager:
    ...
    def __init__(
        ...
        menuBar = tkinter.Frame(self.master, relief=tkinter.RAISED, borderwidth=2)
        ...
        fileBtn.menu.add_command(label="Load LUT (.lut)", command=self.ask_open_file_cb)
        fileBtn.menu.add_command(label="Save LUT (.lut)", command=self.saveLUT)
        ...
        editBtn.menu.add_command(label="Split function", command=self.show_splitDialog)
        editBtn.menu.add_command(
            label="Merge function", command=self.merge_function, state="disabled"
        )

        editBtn.menu.drawchoices = tkinter.Menu(editBtn.menu)
        editBtn.menu.add_cascade(
            label="Draw function...", menu=editBtn.menu.drawchoices
        )
        editBtn.menu.drawchoices.add_command(
            label="ramp", command=(lambda self=self, num=0: self.draw_ramp(num))
        )
        editBtn.menu.drawchoices.add_command(
            label="reversed ramp",
            command=(lambda self=self, num=1: self.draw_ramp(num)),
        )
        editBtn.menu.add_command(label="Flip function", command=self.flip_function_cb)
        editBtn.menu.sizechoices = tkinter.Menu(editBtn.menu)
        editBtn.menu.add_command(
            label="Set new opacity range", command=self.change_yaxis
        )
        editBtn.menu.add_command(label="Add new ISO value", command=self.addIsoVal_cb)
        editBtn.menu.add_cascade(label="Set size of ...", menu=editBtn.menu.sizechoices)
        editBtn.menu.sizechoices.add_command(
            label="dots", command=(lambda self=self, st="dot": self.set_dotsize_cb(st))
        )
        editBtn.menu.sizechoices.add_command(
            label="isovalue dots",
            command=(lambda self=self, st="isodot": self.set_dotsize_cb(st)),
        )
        editBtn.menu.add_command(
            label="Delete selected dot", command=self.del_selected_dot
        )
        ...
        editBtn.menu.add_checkbutton(
            label="Continuous update", var=self.continVar, command=self.set_continuous
        )
        editBtn["menu"] = editBtn.menu
        self.editBtn = editBtn
        self.applylutBtn = tkinter.Button(
            menuBar,
            text="Apply LUT",
        ...
        helpBtn.menu.add_command(
            ...
            command=(lambda self=self, st="description": self.show_help_txt(st)),
        )
        helpBtn.menu.choices = tkinter.Menu(helpBtn.menu)
        helpBtn.menu.add_cascade(label="How to ...", menu=helpBtn.menu.choices)
        helpBtn.menu.choices.add_command(
            label="Add dot/shape",
            command=(lambda self=self, st="addDotShape": self.show_help_txt(st)),
        )
        helpBtn.menu.choices.add_command(
            label="Delete dot",
            command=(lambda self=self, st="deleteDot": self.show_help_txt(st)),
        )
        helpBtn.menu.choices.add_command(
            label="Move shape",
            command=(lambda self=self, st="moveShape": self.show_help_txt(st)),
        )
        helpBtn.menu.choices.add_command(
            label="Set color",
            command=(lambda self=self, st="setColor": self.show_help_txt(st)),
        )
    ...
    def execute(self, result):
        ...
        if result == "OK":
            ...
            if (
                entry_minval == self.parent_interval[0]
                and entry_maxval == self.parent_interval[1]
            ):
                ...
                print(
                    "Error: in 'Split dialog' (maxval-minval) must be greater or equal to 50 ; %d, %d"
                    % (entry_maxval, entry_minval)
                )
                return
            elif d1 != 0 and d1 < 49:
                print(
                    "Error: in 'Split dialog' length of the resulting intervals must be greater or equal to 50(%d-%d < 50)"
                    % (entry_minval, self.parent_interval[0])
                )
                return
            elif d3 != 0 and d3 < 49:
                print(
                    "error: in 'Split dialog' length of the resulting intervals must be greater or equal to 50(%d-%d < 50)"
                    % (self.parent_interval[1], entry_maxval)
                )
                return
            else:
                # print "splitting"
    ...
    def change_yaxis(self):
        ...
        if result == "OK":
            ...
            if new_y != self.ymaxval:
                ...
                self.entry_y.configure(
                    validate={"validator": "integer", "min": 0, "max": new_y}
                )
                self.ymaxval = new_y
    ...
    def place_ISOentries(self, f):
        ...
        self.entry_isoVal = Pmw.EntryField(
            f,
            labelpos="w",
            entry_font=font,
            entry_width=ew + 1,
            ...
            label_font=font,
            value=str(xminval),
            validate={"validator": "integer", "min": xminval, "max": xmaxval},
            command=self.iso_value_set,
        )
        self.entry_isoY = Pmw.EntryField(
            f,
            labelpos="w",
            label_text="A",
            entry_width=ew + 1,
            ...
            entry_font=font,
            label_font=font,
            validate={"validator": "integer", "min": 0, "max": self.ymaxval},
            command=self.iso_alpha_set,
        )
        self.entry_isoR = Pmw.EntryField(
            f,
            labelpos="w",
            label_text="R",
            entry_width=ew,
            ...
            entry_font=font,
            label_font=font,
            validate={"validator": "real", "min": 0, "max": 1.0},
            command=self.iso_color_set,
        )
        self.entry_isoG = Pmw.EntryField(
            f,
            labelpos="w",
            label_text="G",
            entry_width=ew,
        ...
        self.entry_isoB = Pmw.EntryField(
            f,
            labelpos="w",
            label_text="B",
            entry_width=ew,
    ...
    def addIsoVal_cb(self):
        ...
        Pmw.EntryField(
            ...
            labelpos="w",
            entry_width=ew,
            entry_textvariable=self.isoVal,
            label_text="ISO val:",
            # label_font = font, entry_font = font,
            validate={"validator": "integer", "min": 0, "max": self.xmaxval},
            value=0,
    ...
    def split_cb(self, entry_minval, entry_maxval, split=1):
        ...
        try:
            ...
            print(
                "ValueError: interval", parent_interval, "is not in self.intervals_list"
            )
            return
        if entry_minval == parent_interval[0]:
            ...
            intervals = [
                (parent_interval[0], entry_minval - 1),
                (entry_minval, entry_maxval),
            ]
            curr_interval = ind + 1
        else:
        ...
        self.canvas_list[curr_interval].itemconfigure("outline", outline="blue")
    ...
    def split_function(self, old_lut, interval, new_ind):
        ...
        if len(new_values) != 0:
            ...
            for val in new_values:
                new_points.append(((val - min_val) * sclx + left, old_lut.points[i][1]))
                i = i + 1
        ...
        if v_len == 4:
            ...
            ##                  return
                ...
                new_values.insert(
                    2, int(round((new_points[2][0] - left) / sclx)) + min_val
                )
                new_shapes = [0, 1, 3, 4]
        else:
            ...
            if new_shapes[2] - new_shapes[1] == 1:
                if new_values[2] - new_values[1] <= 1:
                    ...
                    new_points.insert(
                        ...
                        (
                            new_points[1][0]
                            + (new_points[2][0] - new_points[1][0]) / 2,
                            new_points[1][1]
                            + (new_points[2][1] - new_points[1][1]) / 2,
                        ),
                    )
                    new_values.insert(
                        2, int(round((new_points[2][0] - left) / sclx)) + min_val
        ...
        lut.alpha_arr[:] = old_lut.alpha_arr[min_arrind : max_arrind + 1]
        lut.color_arr[:] = old_lut.color_arr[min_arrind : max_arrind + 1]
        lut.alpha_arr[:] = old_lut.alpha_arr[min_arrind : max_arrind + 1]
    ...
    def ask_save_file(self, filetypes, title):
        """pop up tkFileDialog"""
        file = tkinter.filedialog.asksaveasfilename(filetypes=filetypes, title=title)
        return file
    ...
    def load_file(self, file):
        ...
        for n in range(nintervals):
            intervals.append(
                (atoi(intervals_str[n * 2]), atoi(intervals_str[n * 2 + 1]))
            )
            sizes.append(
                (
                    atoi(sizes_str[n * 3]),
        ...
        if xmaxval != self.xmaxval:
            if self.viewer.hasGui:
                text = (
                    "WARNING: number of LUT entries in\n%s\n is %d,\n current number of LUT entries is %d.\nLoad new LUT?"
                    % (file, xmaxval + 1, self.xmaxval + 1)
                )
                ...
                print(
                    "WARNING: number of LUT entries in %s is %d, current number of LUT entries is %d."
                    % (file, xmaxval + 1, self.xmaxval + 1)
                )
        ...
        for interval in intervals:
            ...
            if d != 1:
                lut.calculate_points(values[i], [x / d for x in alphas[i]])
            else:
                lut.calculate_points(values[i], alphas[i])
            lut.shapes = shapes[i]
            colors_size = (len(colors[i]) / 3, 3)
            # print "colors_size: ", len(colors[i]), colors_size
            lut.color_arr = Numeric.reshape(Numeric.array(colors[i], "f"), colors_size)
    ...
    def saveTF(self):
        ...
        filename = self.ask_save_file([("TF Files", "*.clu")], "Save Transfer Function")
        if filename:
    ...
    def set_dotsize_cb(self, type):
        ...
        dotsizeDialog = Pmw.Dialog(
            self.master,
            buttons=("OK",),
            defaultbutton="OK",
            title=text,
        )  # command=self.ok_dotsize)
        interior = dotsizeDialog.interior()

        sizescale = tkinter.Scale(
            interior,
    ...
    def del_selected_dot(self):
        ...
        if tags[0] == "isodot":
            ...
            if lut.curr_ind - 1 in lut.shapes and lut.curr_ind + 1 in lut.shapes:
                lut.deleteShape()
            else:
                lut.deleteDot()
    ...
    def show_help_txt(self, txt):
        text_opts = {
            "addDotShape": AddDotShapeTxt,
            "deleteDot": DeleteDotTxt,
            "moveShape": MoveShapeTxt,
            "setColor": SetColorTxt,
            "description": DescriptionText,
        }
        opt = text_opts[txt]
        helpDialog = Pmw.TextDialog(
            self.master,
    ...
    def merge_function(self):
        ...
        for i in range(len(points)):
            new_points.append(((values[i] - self.xminval) * sclx + left, points[i][1]))
        new_points.append((right, bott))
        lut.points = new_points
        lut.shapes = shapes
...
class LUT:
    ...
    def __init__(
        ...
        self.canvas = tkinter.Canvas(
            master, width=width, height=height, relief=tkinter.FLAT, borderwidth=2
        )
        c = self.canvas
        ...
        c.create_rectangle(
            3, 3, width + 3, height + 3, outline="", width=4, fill="", tags="outline"
        )
        c.create_rectangle(
            self.left,
            self.top,
        ...
        c.create_text(
            10, self.bott + 10, text=str(xminval), anchor=tkinter.W, font=self.font
        )
        c.create_text(
            10,
            self.top - 7,
    ...
    def compute_alpha_ramp(self, xminval, num_of_entries, arr_len, reverse=0):
    ...
    def draw_initshape(
        self, xminval=None, xmaxval=None, num_of_entries=None, gray=1, reverse=0
    ):
        if xmaxval is None:
            xmaxval = self.xmaxval
        if xminval is None:
    ...
    def grayRamp(self, xminval=None, xmaxval=None):
        ...
        if xmaxval is None:
            xmaxval = self.xmaxval
        n = self.num_of_entries / 256.0
        ##          print 'n=',n
        j = int(xminval / n)
        k = 0
        for i in range(xminval, xmaxval + 1):
            if i % n == 0 and i != xminval:
                j = j + 1
            c = j / 255.0
    ...
    def update_linetags(self, ind1, ind2):
        ...
        for i in line_ind:
            self.canvas.itemconfigure(
                "line%d" % (i,), tags=("line", "line%d" % (i + ind2,))
            )
    ...
    def update_dottags(self, ind1, ind2):
        ...
        for i in dot_ind:
            self.canvas.itemconfigure(
                "dot%d" % (i,), tags=("dot", "dot%d" % (i + ind2,))
            )
        if r_edge:
            self.canvas.itemconfigure(
                "dot%d" % (r_edge,), tags=("dot", "dot%d" % (r_edge + ind2,), "edge")
            )
    ...
    def drawShape(self, event):
        ...
        if self.line_count > 1:
            self.update_linetags(ind + 1, 4)
        if self.dot_count > 1:
            self.update_dottags(ind + 1, 4)
        bott = self.bott
        ...
        self.callbacks["entries"](
            val_x1=val_x1, val_x2=val_x2, val_y=int(val_y)
        )  # , val_r='', val_g='', val_b='')
        self.callbacks["setalpha"](
            [val_x1 + 1, self.alpha_arr[val_x1 + 1 - xminval : val_x2 - xminval]]
        )
    ...
    def drawIsoVal(self, isoVal, isoA, isoRGB):
        ...
        self.isoVals.append({"val": isoVal, "alpha": isoA, "rgb": isoRGB})
        if self.continuous:
            self.callbacks["setalpha"](
                [isoVal, Numeric.array([isoA]).astype(Numeric.Int16)]
            )
    ...
    def removeIsoVal(self):
        ...
        if self.continuous:
            self.callbacks["setalpha"](
                [val, Numeric.array([alpha]).astype(Numeric.Int16)]
            )
    ...
    def update_yaxis(self, ymaxval):
        ...
        for i in range(1, len(self.points) - 1):
            ...
            self.points[i] = (self.points[i][0], self.bott - self.scly * alpha)
        self.ymaxval = ymaxval
        self.redraw()
        self.callbacks["setalpha"]([self.xminval, self.alpha_arr])
    ...
    def mouse1Down(self, event):
        ...
        scalars = [
            ...
            self.values[curr_ind],
            self.values[curr_ind + 1],
        ]
        xminval = self.xminval
        self.alphas = [
            self.alpha_arr[scalars[0] - xminval],
    ...
    def mouse1Move(self, event):
        ...
        if x < next_l[0] + delta:
            ...
            if curr_ind + 1 == len(self.points) - 1:
                x = self.right
            else:
                x = next_r[0] - delta
    ...
    def compute_rgba(self, scalars, alphas):
        ...
        if d != 0:
            ...
            self.alpha_arr[sc0:sc1] = (K * s + C).astype(Numeric.Int16)
            if scalars[1] > xminval or scalars[1] < xmaxval:
                self.color_arr[sc0:sc1] = Numeric.transpose(
                    Numeric.array((Kr * s + Cr, Kg * s + Cg, Kb * s + Cb)).astype("f")
                )
        ...
        if d != 0:
            ...
            self.alpha_arr[sc1:sc2] = (K * s + C).astype(Numeric.Int16)
            if scalars[1] > xminval or scalars[1] < xmaxval:
                self.color_arr[sc1:sc2] = Numeric.transpose(
                    Numeric.array((Kr * s + Cr, Kg * s + Cg, Kb * s + Cb)).astype("f")
                )
        else:
            s = sc1
    ...
    def mouse1Up(self, event):
        ...
        if (
            self.lastx != self.points[curr_ind][0]
            or self.lasty != self.points[curr_ind][1]
        ):
    ...
    def pickIsoVal(self, event):
        ...
        self.callbacks["entries"](
            iso_val=self.isoVals[self.iso_ind]["val"],
    ...
    def moveIsoVal(self, event):
        ...
        if y >= self.bott:
            y = self.bott
        elif y > self.top - 25 and y <= self.top:
            y = self.top
        elif y <= self.top - 25:
            self.removeIsoVal()
            return
    ...
    def check_scalarval(self, value):
        ...
        if value > val_l and value < val_r:
            ...
            if (
                "edge" in self.canvas.gettags("dot%d" % self.curr_ind)
                and self.alpha_arr[x_ind] != 0
            ):
                return 0
            else:
                return 1
    ...
    def move_dot_value(self, value):
        ...
        scalars = [self.values[curr_ind - 1], value, self.values[curr_ind + 1]]
        alphas = [
            self.alpha_arr[scalars[0] - self.xminval],
            round((self.bott - y) / self.scly),
    ...
    def mouse2Down(self, event):
        ...
        xminval = self.xminval
        if not (
            self.last_event == "ButtonRelease-2" and select_shape == self.select_shape
        ):
            # list of alphas of the shape
            self.alpha_list = list(
                self.alpha_arr[first_scal - xminval : last_scal - xminval + 1]
            )
            # color arrays of the shape
            ##              self.rgb = (self.color_arr[first_scal-xminval:
            ##                                         last_scal+1-xminval]*1).astype('f')
            self.rgb = Numeric.array(
                self.color_arr[first_scal - xminval : last_scal + 1 - xminval]
            )
    ...
    def mouse2Move(self, event):
        ...
        c = self.canvas
        if "edge" in c.gettags("dot%d" % (select_shape[-1],)) or "edge" in c.gettags(
            "dot%d" % (select_shape[0],)
        ):
            return
        ...
        if x + self.diffr >= self.next_r:
            if self.next_r == self.right:
                x = self.next_r - self.diffr + rad
            else:
                x = self.next_r - self.diffr - self.sclx + eps + rad
        if x - self.diffl <= self.next_l:
            if self.next_l == self.left:
                x = self.next_l + self.diffl - rad
            else:
                x = self.next_l + self.diffl + self.sclx + eps - rad
        dist = x - self.lastx
        # move current shape
        ...
        x1 = l_line[2] + dist
        x2 = r_line[0] + dist
        c.coords("line%d" % (j - 1,), l_line[0], l_line[1], x1, l_line[1])
        c.coords("line%d" % (i,), x2, r_line[1], r_line[2], r_line[3])
        self.lastx = x
        ...
        if self.counter % 20 == 0:
            ...
            if self.continuous:
                self.callbacks["setcolor"](
                    [new_valx1, self.color_arr[new_valx1 - xminval : end - xminval + 1]]
                )
                self.callbacks["setalpha"](
                    [start_p + xminval, self.alpha_arr[start_p:stop_p]]
                )
            if old_rgba:
                for rgba in old_rgba:
                    v = rgba[0]
    ...
    def mouse2Up(self, event):
        ...
        if self.counter % 20 != 0:
            ...
            if delta < 0:  # the shape moved to the right
                ...
                if end >= r_val:
                    ...
                    if d > 0:
                        self.values[
                            self.select_shape[0] : self.select_shape[-1] + 1
                        ] = list(
                            map(
                                lambda val, d=d: val - d,
                                self.values[
                                    self.select_shape[0] : self.select_shape[-1] + 1
                                ],
                            )
                ...
                return
            self.color_arr[
                new_valx1 - self.xminval : end - self.xminval + 1
            ] = self.rgb[:][:]
        ...
        if self.counter % 20 != 0:
            ...
            if self.continuous:
                self.callbacks["setalpha"](
                    [start_p + self.xminval, self.alpha_arr[start_p:stop_p]]
                )
        if self.continuous:
            self.callbacks["setcolor"](
                [
    ...
    def deleteDot(self):
        ...
        if self.continuous:
            self.callbacks["setalpha"](
                [
                    self.scalars[0],
                    self.alpha_arr[
                        self.scalars[0] - xminval : self.scalars[2] - 1 - xminval
                    ],
                ]
            )
            self.callbacks["setcolor"](
                [
                    self.scalars[0],
                    self.color_arr[
                        self.scalars[0] - xminval : self.scalars[2] - xminval + 1
                    ],
                ]
            )
        if old_rgba:
        ...
        self.callbacks["entries"](
            val_y="0", val_x1="0", val_x2="", val_r="1.0", val_g="1.0", val_b="1.0"
        )
    ...
    def deleteShape(self):
        ...
        # update list of shapes
        if self.update_shapes(curr_ind - 1, -3) == 0:
            return
        ...
        if self.continuous:
            ...
            self.callbacks["setcolor"](
                [l_scalar, self.color_arr[l_scalar - xminval : r_scalar - xminval + 1]]
            )
        if old_rgba:
            for rgba in old_rgba:
                v = rgba[0]
    ...
    def set_colors(self, rgb, scalar1, scalar2, scalar3):
        ...
        if (
            self.last_event == "ButtonRelease-1"
            or self.last_event == "ButtonRelease-2"
            or self.last_event == "B1-Motion"
        ):
            ...
            if d != 0:
                ...
                self.color_arr[scalar1:scalar2] = Numeric.transpose(
                    Numeric.array((Kr * s + Cr, Kg * s + Cg, Kb * s + Cb)).astype("f")
                )
            d = scalar2 - scalar3
            if d != 0:
                ...
                self.color_arr[scalar2:scalar3] = Numeric.transpose(
                    Numeric.array((Kr * s + Cr, Kg * s + Cg, Kb * s + Cb)).astype("f")
                )
    ...
    def set_hue_satur_value(self, hsv):
        ...
        if tags[0] == "isodot":
            ...
            return
        xminval = self.xminval
        if self.last_event == "ButtonRelease-1" or self.last_event == "ButtonRelease-2":
            curr_ind = self.curr_ind
        ...
        if self.continuous:
            self.callbacks["setcolor"](
                [scalar1 + xminval, self.color_arr[scalar1 : scalar3 + 1]]
            )
        if old_rgb:
            for c in old_rgb:
        ...
        self.callbacks["entries"](
            val_r=round(rgb[0], 2), val_g=round(rgb[1], 2), val_b=round(rgb[2], 2)
        )
    ...
    def update_colors(self, scalar1, scalar2):
        ...
        d = scalar2 - scalar1
        if d == 0:
            return [scalar1, scalar2, Numeric.zeros((0, 3), "f")]
        r = (red2 - red1) / d
        g = (green2 - green1) / d
        b = (blue2 - blue1) / d
        s = Numeric.arange(d)
        self.color_arr[scalar1:scalar2] = Numeric.transpose(
            Numeric.array((r * s + red1, g * s + green1, b * s + blue1)).astype("f")
        )
    ...
    def calculate_points(self, values, alphas):
        ...
        for i in range(1, len(values) - 1):
            self.points.append(
                (
                    (values[i] - self.xminval) * self.sclx + self.left,
...
class Colormap(CallbackFunctions):
    def __init__(self, master, file=None):
        ...
        font = "-*-Helvetica-Bold-R-Normal-*-*-160-*-*-*-*-*-*"
        tkinter.Label(master, text="Hue/Saturation", font=font, width=20).grid(
            row=0, column=0, sticky="NW"
        )
        tkinter.Label(master, text="Value", font=font).grid(
