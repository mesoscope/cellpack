...
#
#
# $Header: /opt/cvs/python/packages/share1.5/mglutil/gui/BasicWidgets/Tk/player.py,v 1.18 2009/08/11 18:05:33 sanner Exp $
#
# $Id: player.py,v 1.18 2009/08/11 18:05:33 sanner Exp $
#
#
#
#
...
class Player(tkinter.Frame):
    ...
    def buildForm2(self, titleStr):
        ...
        form = InputForm(
            self.master, self.root, descr=ifd, modal=0, blocking=0, closeWithWindow=1
        )
        form.ifd = ifd
    ...
    def SetMode_cb(self, event=None):
        ...
        if not hasattr(self, "playModeForm"):
            ...
            self.framerateWidget = self.playModeForm.descr.entryByName["framerateTW"][
                "widget"
            ]
            self.startFrameWidget = self.playModeForm.descr.entryByName["startFrameTW"][
                "widget"
            ]
            self.endFrameWidget = self.playModeForm.descr.entryByName["endFrameTW"][
                "widget"
            ]
            self.stepSizeWidget = self.playModeForm.descr.entryByName["stepSizeTW"][
                "widget"
            ]
        else:
