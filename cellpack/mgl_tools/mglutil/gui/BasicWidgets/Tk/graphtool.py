...
class GraphApp:
    ...
    def __init__(self, master=None, callback=None, continuous=1):
        ...
        self.editmenu.add_checkbutton(
            label="Histogram", var=self.histvar, command=self.drawHistogram
        )
        ...
        for v, s in list({0: "Continuous", 1: "MouseButtonUp", 2: "Update"}.items()):
            self.updatemenu.add_radiobutton(
                label=s, var=self.optionType, value=v, command=self.calloption
            )
        if continuous == 1:
        ...
        for v, s in list({0: "Smooth", 1: "Freehand"}.items()):
            self.curvemenu.add_radiobutton(
                label=s, var=self.CurveType, value=v, command=self.curveoption
            )
        ...
        self.balloon.bind(
            self.d1scalewheel,
            "cutoff value for differences in Z xoordinates,small values generate more contours",
        )
        ...
        self.canvas.create_rectangle(
            [(px - 1, py), (px1 + 1, py1)], fill="white", outline="black", width=1
        )
        ...
        x2 = 51
        self.canvas.create_rectangle(
            [(50, 315), (307, 300)], fill="white", outline="black", width=0.5
        )
        for a in grays:
            if x1 > 306:
                ...
                x1 = x2 = 306
            self.canvas.create_rectangle(
                [(x1 + 2.5, 314), (x2 + 2.5, 301)], fill=a, outline=a, width=1
            )
            x1 = x1 + 2.5
            x2 = x2 + 2.5
        # left one
        ...
        y2 = 271
        self.canvas.create_rectangle(
            [(20, 275), (5, 20)], fill="black", outline="black", width=0.5
        )
        for a in grays:
            if y1 > 275:
                y1 = y2 = 275
            self.canvas.create_rectangle(
                [(19, y1 - 2.5), (6, y2 - 2.5)], fill=a, outline=a, width=1
            )
            y1 = y1 - 2.5
            y2 = y2 - 2.5
    ...
    def OnCanvasMouseUp(self, event):
        ...
        if hasattr(self, "curx"):
            ...
            if (
                self.ox,
                self.oy,
            ) not in self.oldpoints:  # not in [self.startpoint,self.endpoint] :
                self.oldpoints.append((self.ox, self.oy))
                if hasattr(self, "curoval"):
                    ...
                    if c in self.oldpoints:
                        ...
                        for co in self.curovals:
                            ...
                            if len(ov_point1) != 0:
                                ...
                                if ov_point == c and ov_point not in [
                                    self.startpoint,
                                    self.endpoint,
                                ]:
            ...
            for c in CtlPoints:
                if c in self.oldpoints:
                    ...
                    for co in self.curovals:
                        ...
                        if len(ov_point1) != 0:
                            ov_point = (
                                int(round(ov_point1[0], 3)) + 2,
                                int(round(ov_point1[1], 3)) + 2,
                            )
    ...
    def draw(self):
            ...
            if self.limit_xcoord != []:
                ...
                if (self.curx, self.cury) not in [self.startpoint, self.endpoint]:
                    if (self.curx, self.cury) < self.limit_xcoord[0]:
                        if (
                            self.curx <= self.limit_xcoord[0][0]
                            and self.cury < self.limit_xcoord[0][1]
                        ):
                    ...
                    if (self.curx, self.cury) > self.limit_xcoord[1]:
                        if (
                            self.curx >= self.limit_xcoord[1][0]
                            and self.cury > self.limit_xcoord[1][1]
                        ):
        ...
        if (dx, dy) not in new1points and (dx, dy) not in [
            self.startpoint,
            self.endpoint,
        ]:
            ...
            if i[0] == 51 or i[0] == 50:
                if i != self.startpoint:
                    if self.startpoint in new1points:
                        ...
                        x = 50
                        y = 275
                        st_oval_1 = self.canvas.find_enclosed(
                            x - 3, y - 3, x + 3, y + 3
                        )
                        ...
                        if st_oval_1:
                            for so in st_oval_1:
                                if so != []:
                                    ...
                                    if (
                                        int(st_oval_coords[0] + 2),
                                        int(st_oval_coords[1] + 2),
                                    ) == self.startpoint:
            ...
            if i[0] == 304 or i[0] == 305:
                if i != self.endpoint:
                    if self.endpoint in new1points:
                        ...
                        x = 305
                        y = 20
                        end_oval_1 = self.canvas.find_enclosed(
                            x - 3, y - 3, x + 3, y + 3
                        )
                        if end_oval_1:
                            for eo in end_oval_1:
                                if eo != []:
                                    ...
                                    if (
                                        int(end_oval_coords[0] + 2),
                                        int(end_oval_coords[1] + 2),
                                    ) == self.endpoint:
        ...
        if (self.curx, self.cury) != self.endpoint:
            self.curoval = self.canvas.create_oval(
                self.curx - 2,
                self.cury - 2,
                self.curx + 2,
                self.cury + 2,
    ...
    def caluculate_ramp(self):
        ...
        # if self.continuous :
        if hasattr(self, "curx"):
            if (self.curx, self.cury) not in self.oldpoints and (
                self.curx,
                self.cury,
            ) not in [self.startpoint, self.endpoint]:
    ...
    def setCallbacks(self, cb):
        ...
        assert cb is None or hasattr(cb, "__call__") or type(cb) is list, (
        ...
        if cb is None:
            ...
            for func in cb:
                assert hasattr(func, "__call__"), (
                    "Illegal callback must be callable. Got %s" % func
                )
    ...
    def resetAll_cb(self):
        ...
        self.reset()
        self.curline = self.canvas.create_line(
            [self.startpoint, self.endpoint], width=1, fill="black"
        )
    ...
    def stepBack_cb(self):
        ...
        if self.history != []:
            if len(self.history) == 1:
                ...
                if self.Smooth:
                    self.curline = self.canvas.create_line(
                        pns, width=1, fill="black", smooth=1
                    )
                else:
    ...
    def getControlPoints(self):
        ...
        if hasattr(self, "curoval"):
            ...
            if len(c) != 0:
                if (int(c[0] + 2), int(c[1] + 2)) not in self.oldpoints and (
                    int(c[0] + 2),
                    int(c[1] + 2),
                ) not in [self.startpoint, self.endpoint]:
    ...
    def setControlPoints(self, points):
        ...
        for (x, y) in points:
            assert x in range(
                50, 306
            ), "coordinates are out of range,x should be in [50,305]"
            assert y in range(
                20, 276
            ), "coordinates are out of range,y should be in [20,275]"
        ...
        if self.histvar.get():
            ...
            for i in range(0, len(c)):
                ...
                r = self.canvas.create_line(
                    [(x1, y1), (x2, y2)], fill="gray70", width=1
                )
