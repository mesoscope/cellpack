...
class MultiListbox(Pmw.ScrolledFrame):
    ...
    def __init__(self, master, lists, **kw):
        ...
        for l, w in lists:
            ...
            lb = Listbox(frame, width=w, bd=0, exportselection=FALSE, bg="white")
    ...
    def _sort(self, e):
        ...
        if direction == 0:
            ...
            if direction == 1:
                b.config(text=str(btnLabel + " " + "\u25B2"), font=self.myFont)
            else:
                b.config(text=str(btnLabel + " " + "\u25BC"), font=self.myFont)
            # sort by col
...
if __name__ == "__main__":
    ...
    for i in range(10):
        mlb.insert(
            END, ("Important Message: %d" % i, "John Doe", "10/10/%04d" % (1900 + i))
        )
