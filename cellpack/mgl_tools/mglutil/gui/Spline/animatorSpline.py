...
class AnimatorSpline(splineGUI):
    ...
    def doit(self, startvalue, endvalue, range_begin, range_end, slopes):
        ...
        if self.startvalue < self.range_begin or self.endvalue > self.range_end:
            print(
                "invalid input,startvalue should be >= range_begin and endvalue<=range_end"
            )
            return
    ...
    def getValueforCoords(self, coords):
        ...
        result = self.range_begin + rng * (
            (self.viewport[1] - float(y)) / (self.viewport[1] - self.viewport[0])
        )
        return result
    ...
    def getValuesForAllFourPoints(self):
        ...
        for p in newlist:
            result = self.range_begin + rng * (
                (self.viewport[1] - p) / (self.viewport[1] - self.viewport[0])
            )
    ...
    def OnLine(self, id, event=None):
        ...
        self.canvas.create_oval(
            x - 2, y - 2, x + 2, y + 2, fill="white", tags=("point_online",)
        )
