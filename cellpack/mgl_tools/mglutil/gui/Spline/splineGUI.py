        ...
        for p, slope in zip(pts, slopes):
            ...
            if sll:  # draw vector on left side of the point
                ...
                lid = self.canvas.create_line(
                    Rx, Ry, rx, ry, fill="black", tags=(tag, uid, "left", "ln")
                )
                tid = self.canvas.create_text(
                    (Rx + 10, Ry + 10),
                    text=round(vy / vx, 1),
                    tags=(tag, uid, "left", "tx"),
                )
            ...
            if slr:
                ...
                lid = self.canvas.create_line(
                    rx, ry, Rx, Ry, fill="black", tags=(tag, uid, "right", "ln")
                )
                tid = self.canvas.create_text(
                    (Rx + 10, Ry + 10),
                    text=round(vy / vx, 1),
                    tags=(tag, uid, "right", "tx"),
                )
    ...
    def startMoveControlPoint(self, cid, index, event):
        ...
        if (round(c.coords(cid)[0] + 2), round(c.coords(cid)[1] + 2)) in rounded_cpts:
            ind_cpt = rounded_cpts.index(
                (round(c.coords(cid)[0] + 2), round(c.coords(cid)[1] + 2))
            )
        if (round(c.coords(cid)[2]), round(c.coords(cid)[3])) in rounded_cpts:
            ind_cpt = rounded_cpts.index(
                (round(c.coords(cid)[2]), round(c.coords(cid)[3]))
            )
        if ind_cpt:
        ...
        if "left" in c.gettags(sid):
            ...
            if self.left_dic != {}:
                ...
                if sid in list(self.left_dic.keys()):
                    ...
                    if (round(px), round(py)) == (
                        round(self.line_coords[0]),
                        round(self.line_coords[1]),
                    ):
            ...
            if x > self.line_coords[2] - 6:
                if y in range(
                    int(round(self.line_coords[3] - 6)),
                    int(round(self.line_coords[3] + 6)),
                ):
            ...
            self.lid = c.create_line(
                (x, y),
                (self.line_coords[2], self.line_coords[3]),
                tags=(tag, uid, "left", "ln"),
            )

            _current_slope_length = sqrt(
                (self.line_coords[2] - x) * (self.line_coords[2] - x)
                + (self.line_coords[3] - y) * (self.line_coords[3] - y)
            )
            _current_slope = (self.line_coords[3] - y) / (self.line_coords[2] - x)
            self.tid = self.canvas.create_text(
                (x + 10, y + 10),
                text=round(_current_slope, 1),
                tags=(tag, uid, "left", "tx"),
            )
            # considering radius of point
            if (round(self.line_coords[2]), round(self.line_coords[3])) in cpts:
                _point_index = cpts.index(
                    (round(self.line_coords[2]), round(self.line_coords[3]))
                )
            else:
                _point_index = cpts.index(
                    (round(self.line_coords[2] - 2), round(self.line_coords[3] - 2))
                )
            ...
            for i in cids:
                ...
                coords_cids_rounded.append(
                    (
                        round(coords_cids[0] + 2),
                        round(coords_cids[1] + 2),
                        round(coords_cids[2]),
                        round(coords_cids[3]),
                    )
                )
                ...
                if "tied" in c.gettags(current_id):
                    ...
                    for l, t in zip(c.find_withtag("ln"), c.find_withtag("tx")):
                        if "right" in c.gettags(l) and "right" in c.gettags(t):
                            ...
                            if (round(c.coords(l)[0]), round(c.coords(l)[1])) == (
                                round(c.coords(current_id)[0] + 2),
                                round(c.coords(current_id)[1] + 2),
                            ):
                                c.delete(l)
                                c.delete(t)
                    ...
                    if hasattr(self, "old_lid"):
                        ...
                        if coords_l != []:
                            if (round(coords_l[0]), round(coords_l[1])) == (
                                round(c.coords(current_id)[0] + 2),
                                round(c.coords(current_id)[1] + 2),
                            ):
                    ...
                    self.right_lid = c.create_line(
                        (self.line_coords[2], self.line_coords[3]),
                        (new_coords[2] - 2, new_coords[3] - 2),
                        tags=(tag, uid, "right", "ln"),
                    )

                    self.right_tid = self.canvas.create_text(
                        (new_coords[2] - 2 + 10, new_coords[3] - 2 + 10),
                        text=round(_current_slope, 1),
                        tags=(tag, uid, "right", "tx"),
                    )
                    slope_vectors.insert(
                        _point_index, ((1, _current_slope), (1, _current_slope))
                    )
                    # r_slope = self.slopes_lengths[_point_index][1]
                    self.slopes_lengths.remove(self.slopes_lengths[_point_index])
                    self.slopes_lengths.insert(
                        _point_index, (_current_slope_length, _current_slope_length)
                    )
                    cb = CallbackFunction(
                        self.startMoveSlopePoint,
                        right_sid,
                        self.right_lid,
                        self.right_tid,
                    )
                    ...
                    if right_sid_coords:
                        rx, ry = (
                            round(right_sid_coords[2]),
                            round(right_sid_coords[3]),
                        )
                        new_p = []
                    ...
                    slope_vectors.insert(
                        _point_index, ((1, _current_slope), right_slope)
                    )
                    r_slope = self.slopes_lengths[_point_index][1]
                    self.slopes_lengths.remove(self.slopes_lengths[_point_index])
                    self.slopes_lengths.insert(
                        _point_index, (_current_slope_length, r_slope)
                    )
        ...
        if "right" in c.gettags(sid):
            ...
            if self.right_dic != {}:
                ...
                if sid in list(self.right_dic.keys()):
                    ...
                    if (round(px), round(py)) == (
                        round(self.line_coords[2]),
                        round(self.line_coords[3]),
                    ):
                        # if x>px:
            ...
            # drawing line and text
            self.lid = c.create_line(
                (self.line_coords[0], self.line_coords[1]),
                (x, y),
                tags=(tag, uid, "right", "ln"),
            )
            _current_slope_length = sqrt(
                (x - self.line_coords[0]) * (x - self.line_coords[0])
                + (y - self.line_coords[1]) * (y - self.line_coords[1])
            )
            _current_slope = (y - self.line_coords[1]) / (x - self.line_coords[0])
            self.tid = c.create_text(
                (x + 10, y + 10),
                text=round(_current_slope, 1),
                tags=(tag, uid, "right", "tx"),
            )
            ...
            if (round(self.line_coords[0]), round(self.line_coords[1])) in cpts:
                _point_index = cpts.index(
                    (round(self.line_coords[0]), round(self.line_coords[1]))
                )
            else:
                _point_index = cpts.index(
                    (round(self.line_coords[0] - 2), round(self.line_coords[1] - 2))
                )
                ...
                if "tied" in c.gettags(current_id):
                    ...
                    for l, t in zip(c.find_withtag("ln"), c.find_withtag("tx")):
                        if "left" in c.gettags(l) and "left" in c.gettags(t):
                            if (round(c.coords(l)[2]), round(c.coords(l)[3])) == (
                                round(c.coords(current_id)[0] + 2),
                                round(c.coords(current_id)[1] + 2),
                            ):
                                c.delete(l)
                    ...
                    if hasattr(self, "left_lid"):
                        ...
                        if coords_l != []:
                            if (round(coords_l[2]), round(coords_l[3])) == (
                                round(c.coords(current_id)[0] + 2),
                                round(c.coords(current_id)[1] + 2),
                            ):
                    ...
                    self.left_lid = c.create_line(
                        (new_coords[2] - 2, new_coords[3] - 2),
                        (self.line_coords[0], self.line_coords[1]),
                        tags=(tag, uid, "left", "ln"),
                    )

                    self.left_tid = self.canvas.create_text(
                        (new_coords[2] - 2 + 10, new_coords[3] - 2 + 10),
                        text=round(_current_slope, 1),
                        tags=(tag, uid, "left", "tx"),
                    )
                    ...
                    cb = CallbackFunction(
                        self.startMoveSlopePoint, left_sid, self.left_lid, self.left_tid
                    )
                    self.canvas.tag_bind(left_sid, "<ButtonPress-1>", cb)
                    cb = CallbackFunction(
                        self.endMoveSlopePoint, left_sid, self.left_lid, self.left_tid
                    )
                    ...
                    self.slopes_lengths.remove(self.slopes_lengths[_point_index])
                    self.slopes_lengths.insert(
                        _point_index, (l_slope, _current_slope_length)
                    )
                    slope_vectors.insert(
                        _point_index, (left_slope, (1, _current_slope))
                    )
    ...
    def createPoint(self, id, event=None):
        ...
        for cid in old_cids:
            if "tied" in c.gettags(cid):
                coords.append(
                    [
                        round(c.coords(cid)[0]),
                        round(c.coords(cid)[1]),
                        round(c.coords(cid)[2]),
                        round(c.coords(cid)[3]),
                    ]
                )
        x = event.x
        y = event.y
    ...
    def tieNormals(self, cid, event=None):
        ...
        if (round(cid_coords[0] + 2), round(cid_coords[1] + 2)) in rounded_cpts:
            ind = rounded_cpts.index(
                (round(cid_coords[0] + 2), round(cid_coords[1] + 2))
            )
    ...
    def find_sids(self, cid):
        ...
        for s in lids:
            ...
            if s in lsids:
                l_lids.append(s)
        for r in r_lids:
            if (round(c.coords(r)[0]), round(c.coords(r)[1])) == (
                round(cid_coords[0] + 2),
                round(cid_coords[1] + 2),
            ):
                current_rlid = r
                for i in r_sids:
                    if (round(c.coords(r)[2]), round(c.coords(r)[3])) == (
                        round(c.coords(i)[0] + 2),
                        round(c.coords(i)[1] + 2),
                    ):
                        current_rsid = i
        for l in l_lids:
            if (round(c.coords(l)[2]), round(c.coords(l)[3])) == (
    ...
    def find_othersid(self, sid):
        ...
        if "left" in c.gettags(sid):
            for l in l_lids:
                ...
                # llid=sid
                if (round(c.coords(l)[0]) - 2, round(c.coords(l)[1] - 2)) == (
                    round(c.coords(sid)[0]),
                    round(c.coords(sid)[1]),
                ) or (round(c.coords(l)[0]), round(c.coords(l)[1])) == (
                    round(c.coords(sid)[0]),
                    round(c.coords(sid)[1]),
                ):
                    current_lid = l
                    for cid in cids_coords:
                        ...
                        if (round(c.coords(l)[2]), round(c.coords(l)[3])) == (
                            ...
                            for r in r_lids:
                                # cid =rlid
                                if (round(cid[0]), round(cid[1])) == (
                                    round(c.coords(r)[0] - 2),
                                    round(c.coords(r)[1] - 2),
                                ):
                                    for p in r_sids:
                                        # rlid=rsid
                                        if (
                                            round(c.coords(r)[2] + 2),
                                            round(c.coords(r)[3] + 2),
                                        ) == (
                                            round(c.coords(p)[2]),
                                            round(c.coords(p)[3]),
                                        ):
        ...
        if "right" in c.gettags(sid):
            for r in r_lids:
                # rlid=rsid
                if (round(c.coords(r)[2] + 2), round(c.coords(r)[3] + 2)) == (
                    round(c.coords(sid)[2]),
                    round(c.coords(sid)[3]),
                ) or (round(c.coords(r)[2]), round(c.coords(r)[3])) == (
                    round(c.coords(sid)[2]),
                    round(c.coords(sid)[3]),
                ):
                    for cid in cids_coords:
                            ...
                            for l in l_lids:
                                ...
                                if (round(c.coords(l)[2]), round(c.coords(l)[3])) == (
                                    ...
                                    for p in l_sids:
                                        # llid=sid
                                        if (
                                            round(c.coords(l)[0]) - 2,
                                            round(c.coords(l)[1] - 2),
                                        ) == (
                                            round(c.coords(p)[0]),
                                            round(c.coords(p)[1]),
                                        ):
...
if __name__ == "__main__":
    ...
    sp = Spline([(0.0, 0.5), (1.0, 0.3)], [(None, (1, -1)), ((1, -1), None)])
