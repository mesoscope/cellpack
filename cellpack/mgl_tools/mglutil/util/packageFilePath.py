...
#
#########################################################################
# $Header: /opt/cvs/python/packages/share1.5/mglutil/util/packageFilePath.py,v 1.54 2009/06/17 00:03:21 vareille Exp $
#
# $Id: packageFilePath.py,v 1.54 2009/06/17 00:03:21 vareille Exp $
...
def setResourceFolder(folderName):
    ...
    if os.path.isdir(resourceFolder) is False:
        try:
            ...
            txt = (
                "Resource Folder can't be created in home directory, now using tmp directory %s %s"
                % (resourceFolder, e)
            )
            warnings.warn(txt)
            ...
            if not os.path.isdir(resourceFolder):
                try:
                    ...
                    txt = (
                        "Running without Resource Folder as it can't be created %s"
                        % resourceFolder
                    )
                    warnings.warn(txt)
                    resourceFolder = None
...
def getResourceFolderWithVersion():
    ...
    from cellpack.mgl_tools.Support.version import __version__
    ...
    if os.path.isdir(lRessourceFolder):
        ...
        try:
            ...
            try:
                ...
                conn.request(
                    "POST",
                    "/cgi-bin/sanner/update_mgltools_version.py",
                    params,
                    headers,
                )
