...
def test_computeSecondaryStructure_Normal():
    ...
    assert (
        hasattr(mol.chains[0], "secondarystructureset")
        and len(mol.chains[0].secondarystructureset) == 10
    )
    assert (
        not hasattr(mol, "_ExtrudeSecondaryStructureCommand__hasSSGeom")
        or mol._ExtrudeSecondaryStructureCommand__hasSSGeom == 0
    )
    mv.deleteMol("1crn")
...
def test_computeSecondaryStructure_fileThenStrideOnMol():
    ...
    assert (
        hasattr(mol.chains[0], "secondarystructureset")
        and len(mol.chains[0].secondarystructureset) == 11
    )
...
def test_computeSecondaryStructure_nofileinfo():
    ...
    c0 = mol.chains[0]
    assert (
        isinstance(c0.secondarystructureset, SecondaryStructureSet)
        and len(c0.secondarystructureset) == 6
    )
    c1 = mol.chains[1]
    assert (
        isinstance(c1.secondarystructureset, SecondaryStructureSet)
        and len(c1.secondarystructureset) == 3
    )
    c2 = mol.chains[2]
    assert (
        isinstance(c2.secondarystructureset, SecondaryStructureSet)
        and len(c2.secondarystructureset) == 4
    )
    c3 = mol.chains[3]
    assert (
        isinstance(c3.secondarystructureset, SecondaryStructureSet)
        and len(c3.secondarystructureset) == 3
    )
    c4 = mol.chains[4]
    assert (
        isinstance(c4.secondarystructureset, SecondaryStructureSet)
        and len(c4.secondarystructureset) == 4
    )
    c5 = mol.chains[5]
    assert (
        isinstance(c5.secondarystructureset, SecondaryStructureSet)
        and len(c5.secondarystructureset) == 2
    )
    c6 = mol.chains[6]
...
def test_computeSecondaryStructure_fileOnMolAndStrideOnMol():
    ...
    mv.computeSecondaryStructure(
        mv.getSelection(), molModes={"protease": "From File", "7ins": "From Stride"}
    )
    prot = mv.Mols[0]
    ins = mv.Mols[1]
    assert prot.hasSS == ["From File"]
    assert isinstance(prot.chains[0].secondarystructureset, SecondaryStructureSet)

    assert isinstance(prot.chains[1].secondarystructureset, SecondaryStructureSet)
...
def test_computeSecondaryStructure_forceFileWhenNoInfo():
    ...
    mv.readMolecule("7ins.pdb")
    mv.select("7ins")
    mv.computeSecondaryStructure(mv.getSelection(), molModes={"7ins": "From File"})
    ins = mv.Mols[0]
    assert ins.hasSS == []
...
def test_DanielBug():
    ...
    mv.add_hGC("fx:::", polarOnly=1, renumber=1, method="noBondOrder", log=0)
    mv.computeS
