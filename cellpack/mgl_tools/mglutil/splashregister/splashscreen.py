...
class SplashScreen:
    ...
    def __init__(self, about, noSplash=False):
        ...
        self.progressBar = ProgressBar(
            master=frame, labelside=None, width=420, height=20, mode="percent"
        )
...
click these buttons again in the future. Thanks."""
            Tkinter.Label(
                frame, text=text, font=(ensureFontCase("helvetica"), 14, "bold")
            ).pack()
            Tkinter.Button(
                frame, text="Register Now", bg="Green", command=self.Register_Now
            ).pack(side="left")
            self.Later_Button = Tkinter.Button(
                frame, text="Remind Me Later", state="disabled", command=self.Later
            )
        ...
        y = (
            self.splash_win.winfo_screenheight() - height
        ) / 4 - self.splash_win.winfo_vrooty()
        if x < 0:
            x = 0
        if y < 0:
            y = 0
