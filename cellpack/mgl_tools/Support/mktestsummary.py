...
def getTestSummaryResults(testroot, testreportdir):
    ...
    for r in reports:
        f1 = os.path.join(
            testroot, testreportdir, "report-" + r + sys.platform + ".txt"
        )
        f2 = os.path.join(
            testroot, testreportdir, "sReport-" + r + sys.platform + ".txt"
        )
    ...
    if len(noreports):
        endstr = (
            "No test report for the following module(s) (tests possibly crashed): \n"
        )
        for f in noreports:
            endstr = endstr + "%s \n" % f
        results["tail"] = endstr
...
def mkSummaryHTML(testroot, testreportdir, filename=None):
    ...
    resfile.write("===========================================================<br>\n")
    resfile.write(
        "<b>Overall Results(%s): ran %d tests, failed %d (%.2f %s) </b><br> \n"
        % (
            results["reportdate"],
            results["numtests"],
            results["numerrors"],
