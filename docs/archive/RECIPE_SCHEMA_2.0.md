...
|------------|------|-------------|-----|-------|
| `name` | string (unique) | Name of the recipe | | If missing, an automatic version number will be appended. |
| `version` | string | Recipe version string | 1.0 | |
| `format_version` | string | Schema format version | |  |
| `bounding_box` | array `[[minX, minY, minZ], [maxX, maxY, maxZ]]` | Bounding box of the packing result |  | |
| `grid_file_path` | string | Grid data file path | | Managed by backend |
...
| Field Path | Type | Description | Default Value | Notes |
|------------|------|-------------|-----|-------|
| `color` | array (RGB 0–1) | RGB color of object | | `[number, number, number]` |
...
| `place_method` | string(enum) | Method used for packing the object | `jitter` | Accepts a method from `jitter`, `spheresSST` |
...
| `gradient` | string | Linked gradient name | | Must exist in `gradients` |
| `representations.mesh.path` | string | Mesh file path | | Managed backend |
| `representations.mesh.name` | string | Mesh filename | | Tied to the object |
| `representations.mesh.format` | string | Mesh file format | | Tied to the object |
| `representations.mesh.coordinate_system` | string (`left` or `right`) | Depend on generating software | `left` | |
...
| Field Path | Type | Description | Default Value | Notes |
|------------|------|-------------|-----|-------|
| `description` | string | Description of gradient | | For user clarity |
| `mode` | string(enum) | Gradient method | | Defined by gradient. Accepts a mode from `"x"`, `"y"`, `"z"`, `"vector"`, `"radial"`, `"surface"` |
| `pick_mode` | string(enum) | Gradient sampling method | | Accepts a pick mode from `"max"`, `"min"`, `"rnd"`, `"linear"`, `"binary"`, `"sub"`, `"reg"` |
| `weight_mode` | string(enum) | Gradient weighting function | | Accepts a weight mode from `"linear"`, `"square"`, `"cube"`, `"power"`, `"exponential"` |
| `reversed` | boolean | Reverse gradient direction | | |
| `invert` | boolean | Invert gradient values | |  |
| `weight_mode_settings.decay_length` | number | Decay rate parameter | | Controls gradient falloff |
| `mode_settings.object` | string | Reference object | | Links to a mesh object |
...
| `count` | number (integers >= 0) | Number of objects | 5? | Range? Validation depending on the object type? e.g. `nucleus`|
| `priority` | number | Packing priority |  | e.g. -1 |
| `regions.interior` | array | Interior contents of object  | | Validation? e.g. `peroxisome` can't be placed inside `nucleus` |
